...
export type { ColorValue } from './colorValue.js';
...
/**
 * Persisted per-character desk assignment, keyed by character id. Kanban stores this in
 * the runtime project config so an agent keeps its desk across reloads.
 */
export type AgentSeatAssignments = Record<
  number,
...
export interface OfficeLayout {
  ...
  pets?: PlacedPet[];
  /** NPCs placed in the office. Optional for backward-compat; migrateLayout coerces to []. */
  npcs?: PlacedNpc[];
...
export interface Character {
  ...
  bubbleTimer: number;
  /** Roleplay speech-bubble text (D6 `say`), or undefined if none. */
  speechText?: string;
  /** Countdown timer (seconds) for the speech bubble; clears speechText at 0. */
  speechTimer?: number;
  ...
  agentName?: string;
  /** Roleplay: id of the character this NPC is currently speaking to. While set
   *  (and its speech bubble is up), a conversation tether is drawn between them. */
  talkingToId?: number;
  ...
  // -- NPCs --
  /** True if this is a persistent layout NPC instead of a live agent */
  isNpc?: boolean;
  /** NPC script loop */
  actionQueue?: NpcAction[];
  /** Index of current executing script in actionQueue */
  scriptIndex?: number;
  /** Whether the current action has been initialized */
  actionStarted?: boolean;

  // -- Control --
  /** Target tile pinned by user control (disables auto-return-to-seat) */
  pinnedTile?: { col: number; row: number };
  /** Whether the agent is an external scan (no PTY terminal attached) */
  isExternal?: boolean;
}
...
export type NpcAction =
  | { kind: 'walkTo'; col: number; row: number }
  | { kind: 'typeAtSeat'; seconds: number }
  | { kind: 'wander'; seconds: number }
  | { kind: 'patrol'; waypoints: Array<{ col: number; row: number }> };
