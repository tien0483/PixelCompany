...
import type { OfficeLayout, PlacedNpc } from '../types.js';
...
test('OfficeState.pushActivity dedups consecutive duplicates and drops idle/empty text', () => {
  ...
  // Idle / waiting whole-status lines are dropped (Agent Name: status).
  state.pushActivity('Alice: idle');
  state.pushActivity('Bob: waiting');
  state.pushActivity('Carol: waiting for input');
  assert.equal(state.activityFeed.length, 0);
