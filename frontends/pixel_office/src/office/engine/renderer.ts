import type { ColorValue } from '../colorValue.js';
import {
  AGENT_TETHER_COLOR,
  AGENT_TETHER_DASH,
  AGENT_TETHER_DASH_SPEED,
  AGENT_TETHER_LINE_WIDTH,
...
export function renderTileGrid(
  ...
  const tmCols = tileMap[0]?.length ?? 0;
  const layoutCols = cols ?? tmCols;

  // Floor tiles + wall base color
  for (let r = 0; r < tmRows; r++) {
    const rowTiles = tileMap[r];
    if (!rowTiles) continue;
    for (let c = 0; c < tmCols; c++) {
      const tile = rowTiles[c];

      // Skip VOID tiles and ragged rows entirely (transparent)
...
/**
 * Draw dashed "tether" lines linking related agents: each sub-agent to its
 * parent (via `subagentMeta`) and each teammate to its lead (via `leadAgentId`).
 * Runs ABOVE the floor/furniture and BELOW the characters — call it right before
 * `renderScene`. Uses the same world→screen projection as the entity pass
 * (`offset + ch.x/ch.y * zoom`). The dash offset animates over time for a
 * "message flowing" feel.
 *
 * @internal
 */
export function drawAgentLinks(
  ctx: CanvasRenderingContext2D,
  ...
  // [childId, parentId] pairs — sub-agents first, then teammates → lead.
  const links: Array<[number, number]> = [];
  for (const [subId, meta] of subagentMeta) {
    links.push([subId, meta.parentAgentId]);
  }
  for (const [id, ch] of characters) {
    if (ch.leadAgentId !== undefined) links.push([id, ch.leadAgentId]);
    // Roleplay conversation tether: link a speaker to whoever they're addressing.
    if (ch.talkingToId !== undefined) links.push([id, ch.talkingToId]);
  }
  if (links.length === 0) return;

  ctx.save();
  ctx.strokeStyle = AGENT_TETHER_COLOR;
  ctx.lineWidth = AGENT_TETHER_LINE_WIDTH;
  ctx.setLineDash(AGENT_TETHER_DASH);
  // Animate the dash so the line reads as a live channel between the two agents.
  ctx.lineDashOffset = -((performance.now() / 1000) * AGENT_TETHER_DASH_SPEED);

  for (const [childId, parentId] of links) {
    const child = characters.get(childId);
    const parent = characters.get(parentId);
    if (!child || !parent) continue;
    // Don't tether an agent that is dissolving away.
    if (child.matrixEffect === 'despawn' || parent.matrixEffect === 'despawn') continue;

    const cx = offsetX + child.x * zoom;
    const cy = offsetY + child.y * zoom;
    const px = offsetX + parent.x * zoom;
    const py = offsetY + parent.y * zoom;
...
export function renderFrame(
  ...
  // Agent tether links (above floor/furniture, below characters — drawn just
  // before the z-sorted entity pass).
  if (selection && subagentMeta) {
    drawAgentLinks(ctx, selection.characters, subagentMeta, offsetX, offsetY, zoom);
  }
