...
export class OfficeState {
  ...
  /**
   * Bounded, human-readable log of meaningful office events (FIX-9 subtitle
   * feed). Newest entry is last. Populated via {@link pushActivity}, read by the
   * ActivityFeed component. Capped at ACTIVITY_FEED_MAX (oldest dropped).
   */
  activityFeed: string[] = [];
  ...
  constructor(layout?: OfficeLayout) {
    ...
    this.rebuildNpcsFromLayout(this.layout);
  }
  ...
  rebuildFromLayout(layout: OfficeLayout, shift?: { col: number; row: number }): void {
    ...
    for (const pet of this.pets) {
      if (
        ...
        const spawn = this.randomWalkableTile();
        if (spawn) {
    ...
    this.rebuildNpcsFromLayout(layout);
  }

  /** Pick a random walkable tile, or null when the layout has no walkable space. */
  private randomWalkableTile(): { col: number; row: number } | null {
    if (this.walkableTiles.length === 0) return null;
    return this.walkableTiles[Math.floor(Math.random() * this.walkableTiles.length)] ?? null;
  }
  ...
  private pickFromSeats(seatUids: string[], electronicsTiles: Set<string>): string | null {
    ...
    const pick = (pool: string[]): string | null =>
      pool.length > 0 ? (pool[Math.floor(Math.random() * pool.length)] ?? null) : null;
    return pick(pcSeats) ?? pick(otherSeats);
  }
  ...
  private pickDiversePalette(): { palette: number; hueShift: number } {
    ...
    for (const ch of this.characters.values()) {
      ...
      if (ch.palette < paletteCount) counts[ch.palette] = (counts[ch.palette] ?? 0) + 1;
    }
  ...
  addAgent(
    ...
    if (seatId) {
      ...
      const spawn = this.randomWalkableTile() ?? { col: 1, row: 1 };
  ...
  clearAllAgents(): void {
    // Keep NPCs, but remove all real agents (id >= 0) and subagents (id < 0)
    ...
    // Unoccupy seats that were assigned to agents
    for (const [seatId, seat] of this.seats.entries()) {
      if (seat.assigned) {
        // We need to keep it assigned if an NPC is sitting there.
        let isNpcOccupied = false;
        for (const ch of this.characters.values()) {
          if (ch.isNpc && ch.seatId === seatId) {
            isNpcOccupied = true;
            break;
          }
        }
  ...
  /**
   * Replay a matrix spawn flash on an existing character (shift-change visual)
   * without removing them from the floor.
   */
  flashSpawnEffect(id: number): void {
    const ch = this.characters.get(id);
    if (!ch || ch.isNpc || ch.isSubagent) return;
  ...
  addSubagent(parentAgentId: number, parentToolId: string): number {
    ...
    let spawn = this.walkableTiles[0] ?? { col: parentCol, row: parentRow };
          ...
          if (autoOnTiles.has(`${item.col + dc},${item.row + dr}`)) {
            ...
            if (onType !== item.type) {
              ...
              if (frames && frames.length > 1) {
                onType = frames[animFrame % frames.length] ?? onType;
              }
  ...
  showSpeech(id: number, text: string, durationSec: number): void {
    const ch = this.characters.get(id);
    if (!ch) return;
    ch.speechText = text;
  ...
  /**
   * Append a human-readable line to the activity feed (FIX-9 subtitle). Drops
   * empty/whitespace text and anything that resolves to idle/waiting, and
   * collapses consecutive duplicates so a repeated event never spams the feed.
   * The feed is capped at ACTIVITY_FEED_MAX (oldest line dropped first).
   */
  pushActivity(text: string): void {
    ...
    // Whole-status match for 'idle' and 'waiting' (format: "Agent Name: status")
    if (/:\s*(idle|waiting(\s+for\s+input)?)$/.test(lower)) return;

    // Collapse consecutive duplicates.
    if (this.activityFeed[this.activityFeed.length - 1] === trimmed) return;
    this.activityFeed.push(trimmed);
    if (this.activityFeed.length > ACTIVITY_FEED_MAX) {
      this.activityFeed.shift();
    }
  }

  /** Clear the activity feed — e.g. when the office empties after all agents quit. */
  clearActivity(): void {
    this.activityFeed = [];
  }
  ...
  addPet(placedPet: PlacedPet): void {
    ...
    const spawn = this.randomWalkableTile();
    if (!spawn) return; // no spawn space — silently drop
  ...
  /**
   * Add an NPC to the live runtime. Spawns at a uniformly-random walkable tile if no seat.
   * Mirror in `this.layout.npcs` so debounced saveLayout serialises the roster.
   */
  addNpc(placedNpc: PlacedNpc): void {
    if (this.characters.has(placedNpc.id)) return; // de-dupe

    let spawn = this.randomWalkableTile();
    if (!spawn) return; // no spawn space — silently drop
    const ch = createCharacter(
      placedNpc.id,
      placedNpc.palette,
      placedNpc.seatId || null,
      null,
      placedNpc.hueShift,
    );

    if (placedNpc.seatId) {
      const seat = this.seats.get(placedNpc.seatId);
      if (seat) {
        seat.assigned = true;
        ch.seatId = placedNpc.seatId;
        spawn = { col: seat.seatCol, row: seat.seatRow };
      }
    }
    ...
    ch.isNpc = true;
    ch.actionQueue = placedNpc.script;
    ch.scriptIndex = 0;

    this.characters.set(placedNpc.id, ch);
    this.syncLayoutNpcs();
  }

  /** Remove an NPC by id. Idempotent. */
  ...
  /**
   * Reconcile `this.characters` (NPCs only) to match the layout's placed-npc roster.
   */
  private rebuildNpcsFromLayout(layout: OfficeLayout): void {
    const placed = layout.npcs ?? [];
    const placedIds = new Set(placed.map((n) => n.id));

    // 1. Remove npcs no longer in layout
    const toRemove: number[] = [];
    for (const [id, ch] of this.characters) {
      if (ch.isNpc && !placedIds.has(id)) {
        toRemove.push(id);
      }
    }
    ...
    // 2. Add npcs that exist in layout but not in runtime
    for (const n of placed) {
      if (!this.characters.has(n.id)) {
        this.addNpc(n);
      }
    }
    this.syncLayoutNpcs();
  ...
  /**
   * Re-export the current npc roster into `this.layout.npcs`.
   */
  private syncLayoutNpcs(): void {
  ...
  update(realDt: number): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      if (ch.speechTimer !== undefined && ch.speechTimer > 0) {
        ch.speechTimer -= dt;
        if (ch.speechTimer <= 0) {
          ch.speechText = undefined;
          ch.speechTimer = 0;
          ch.talkingToId = undefined;
        }
      }
    }
