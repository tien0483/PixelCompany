...
interface OfficeCanvasProps {
  ...
  /**
   * Called when the user drags an agent onto a different desk. Upstream pixel-agents
   * pushed this over its own transport; Kanban persists it in the runtime project config.
   */
  onSeatsPersist?: (seats: AgentSeatAssignments) => void;
  isEditMode: boolean;
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (officeState.selectedAgentId !== null) {
            ...
            if (seatId) {
              ...
              if (seat && selectedCh) {
                if (selectedCh.seatId === seatId) {
                  ...
                  const seats: AgentSeatAssignments = {};
        ...
        // Clicked empty space — deselect or walk to tile
        if (isControlMode) {
          const tile = screenToTile(e.clientX, e.clientY);
          if (tile) {
            const ch = officeState.characters.get(officeState.selectedAgentId);
            if (ch && !ch.isSubagent) {
              const success = officeState.walkToTile(
                officeState.selectedAgentId,
                tile.col,
                tile.row,
              );
              if (success) {
                ch.pinnedTile = { col: tile.col, row: tile.row };
              }
            }
          }
        } else {
    ...
    [officeState, onClick, onSeatsPersist, screenToWorld, screenToTile, isEditMode, isControlMode],
  );
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (officeState.selectedAgentId !== null) {
        const tile = screenToTile(e.clientX, e.clientY);
        if (tile) {
          const success = officeState.walkToTile(officeState.selectedAgentId, tile.col, tile.row);
          if (success && isControlMode) {
            const ch = officeState.characters.get(officeState.selectedAgentId);
            if (ch && !ch.isSubagent) {
              ch.pinnedTile = { col: tile.col, row: tile.row };
            }
