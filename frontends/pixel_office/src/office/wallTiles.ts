...
function buildWallMask(col: number, row: number, tileMap: TileTypeVal[][]): number {
  ...
  const isWall = (r: number, c: number): boolean => tileMap[r]?.[c] === TileType.WALL;

  let mask = 0;
  if (row > 0 && isWall(row - 1, col)) mask |= 1; // N
  if (col < tmCols - 1 && isWall(row, col + 1)) mask |= 2; // E
  if (row < tmRows - 1 && isWall(row + 1, col)) mask |= 4; // S
  if (col > 0 && isWall(row, col - 1)) mask |= 8; // W
  return mask;
}
...
export function getWallInstances(
  ...
  for (let r = 0; r < tmRows; r++) {
    ...
    for (let c = 0; c < tmCols; c++) {
      if (rowTiles[c] !== TileType.WALL) continue;
