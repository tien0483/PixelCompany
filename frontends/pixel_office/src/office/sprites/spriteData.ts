...
export function getCharacterSprites(paletteIndex: number, hueShift = 0): CharacterSprites {
  ...
  if (char) {
    ...
    // A well-formed atlas has 7 frames per direction; a short row degrades to a
    // transparent placeholder rather than crashing the render loop.
    const blank = emptySprite(16, 32);
    const at = (frames: SpriteData[], i: number): SpriteData => frames[i] ?? blank;

    sprites = {
      walk: {
        [Dir.DOWN]: [at(d, 0), at(d, 1), at(d, 2), at(d, 1)],
        [Dir.UP]: [at(u, 0), at(u, 1), at(u, 2), at(u, 1)],
        [Dir.RIGHT]: [at(rt, 0), at(rt, 1), at(rt, 2), at(rt, 1)],
        [Dir.LEFT]: [flip(at(rt, 0)), flip(at(rt, 1)), flip(at(rt, 2)), flip(at(rt, 1))],
      },
      typing: {
        [Dir.DOWN]: [at(d, 3), at(d, 4)],
        [Dir.UP]: [at(u, 3), at(u, 4)],
        [Dir.RIGHT]: [at(rt, 3), at(rt, 4)],
        [Dir.LEFT]: [flip(at(rt, 3)), flip(at(rt, 4))],
      },
      reading: {
        [Dir.DOWN]: [at(d, 5), at(d, 6)],
        [Dir.UP]: [at(u, 5), at(u, 6)],
        [Dir.RIGHT]: [at(rt, 5), at(rt, 6)],
        [Dir.LEFT]: [flip(at(rt, 5)), flip(at(rt, 6))],
      },
    };
  } else {
