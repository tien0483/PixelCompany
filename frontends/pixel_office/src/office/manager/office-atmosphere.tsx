...
import type { RuntimeManagerState } from "@/runtime/types";
...
/**
 * Turns Manager's usage pressure into ambient light.
 *
 ...
 */
export function OfficeAtmosphere({ manager }: { manager: RuntimeManagerState }): ReactElement | null {
	if (manager === null || manager.pressure <= DIM_START_PRESSURE) {
		return null;
	}
	const intensity = (manager.pressure - DIM_START_PRESSURE) / (1 - DIM_START_PRESSURE);
