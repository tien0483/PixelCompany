/**
 * Vendored from pixel-agents (MIT, Pablo De Lucca) — see ./LICENSE.
 *
 * Trimmed copy of webview-ui/src/constants.ts. Grid, animation, camera, zoom and
 * color tuning are kept verbatim so the engine behaves identically. Dropped:
 * the standalone server's HTTP endpoints (scenarios, recording, agent-names),
 * the script-editor and replay constants, the VisualColorPicker geometry, the
 * notification/permission sound tuning, and the version-notice timings — none of
 * those subsystems are ported.
 */
...
/** Suggested filename when exporting the office layout. */
