...
import type { RuntimeManagerHookLogs, RuntimeManagerServerLogs } from "@/runtime/types";
...
interface ManagerLogsViewProps {
	online: boolean;
}
 ...
 */
export function ManagerLogsView({ online }: ManagerLogsViewProps): ReactElement {
	const [tab, setTab] = useState<LogsTab>("server");
	const [serverLogs, setServerLogs] = useState<RuntimeManagerServerLogs | null>(null);
	const [hookLogs, setHookLogs] = useState<RuntimeManagerHookLogs | null>(null);
	...
	const load = async () => {
		...
		try {
			...
			const [server, hooks] = await Promise.all([
				trpc.manager.serverLogs.query({ limit: 80 }),
				trpc.manager.hookLogs.query({ limit: 40 }),
			]);
	...
	return (
		<div className="flex min-h-0 flex-1 flex-col overflow-hidden" data-testid="manager-logs-view">
