...
import type { RuntimeManagerPack } from "@/runtime/types";
...
export function TrainingPacksPanel({ online }: { online: boolean }): ReactElement | null {
	const [packs, setPacks] = useState<RuntimeManagerPack[] | null>(null);
	...
	const load = useCallback(async () => {
		try {
			const result = await getRuntimeTrpcClient(null).manager.packs.query();
	...
	const toggle = useCallback(
		async (pack: RuntimeManagerPack) => {
			setPendingName(pack.name);
			setError(null);
			try {
				const result = await getRuntimeTrpcClient(null).manager.setPackEnabled.mutate({
					name: pack.name,
