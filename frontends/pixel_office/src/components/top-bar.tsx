...
export function TopBar({
	...
	/**
	 * The desktop cluster is split into a labelled group and an icon-only group.
	 * The divider between them only makes sense when both sides render something,
	 * and with no project selected App withholds every one of these handlers.
	 */
	const hasViewButtons = Boolean(
		onToggleOffice || onToggleDocs || onToggleLearning || onToggleUnderstand,
	);
	const hasToolButtons = Boolean(
		onOpenStack ||
			onOpenCleanup ||
			onToggleTerminal ||
	...
	return (
		...
		>
			...
			<div className="flex flex-nowrap items-center h-10 pr-0.5 shrink-0">
				{/* Desktop: view toggles, then icon-only tools */}
				{!isMobile ? (
					<>
				...
				{/* Mobile: office + tool buttons (icon-only) */}
				{isMobile ? (
					<>
						{onToggleOffice ? (
							<Button
