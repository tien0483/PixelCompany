...
export interface FeatureShelfViewProps {
	...
	select: (feature: RuntimeJackedFeature) => boolean;
	/** Rendered above the list — used by Training to host packs. */
	header?: ReactNode;
	testId: string;
}

function featureKey(feature: RuntimeJackedFeature): string {
 ...
 * same interaction over a different slice of `jacked.features`, which the runtime
 * already streams in full — so a shelf needs no fetch of its own, and toggling reuses
 * the existing `jacked.setFeatureEnabled` procedure.
 */
export function FeatureShelfView({
	...
	const features = useMemo(() => {
		const matching = (jacked?.features ?? []).filter(select);
	...
	const toggle = useCallback(
		async (feature: RuntimeJackedFeature) => {
			const key = featureKey(feature);
			setPendingKey(key);
			setError(null);
			try {
				const result = await getRuntimeTrpcClient(null).jacked.setFeatureEnabled.mutate({
