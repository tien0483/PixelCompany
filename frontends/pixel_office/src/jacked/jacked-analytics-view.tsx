...
import type { RuntimeJackedUsageOverview } from "@/runtime/types";
import { Button } from "@/components/ui/button";
import { MANAGER_LABELS } from "@/jacked/manager-labels";
import { getRuntimeTrpcClient } from "@/runtime/trpc-client";

interface JackedAnalyticsViewProps {
	online: boolean;
}
 ...
 */
export function JackedAnalyticsView({ online }: JackedAnalyticsViewProps): ReactElement {
	const [days, setDays] = useState(1);
	const [overview, setOverview] = useState<RuntimeJackedUsageOverview | null>(null);
	...
	const load = async (nextDays: number = days) => {
		...
		setError(null);
		try {
			const result = await getRuntimeTrpcClient(null).jacked.usageOverview.query({ days: nextDays });
			setOverview(result);
			if (result === null) {
	...
	return (
		<div className="flex min-h-0 flex-1 flex-col overflow-hidden" data-testid="jacked-analytics-view">
