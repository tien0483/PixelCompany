...
export function JackedAccountActions({
	...
	const offlineReason = !online ? "Jacked is offline — reconnect to use seat actions" : busy ? "Working…" : null;
	...
	return (
		<div className="mt-1 flex flex-wrap items-center gap-0.5" data-testid={`jacked-account-actions-${account.id}`}>
			{isClaude ? (
				<>
			...
			<ActionTooltip content={offlineReason ?? "Remove this account from jacked"}>
				<Button
					variant="ghost"
					size="sm"
					disabled={disabled}
			...
			>
				...
				<AlertDialogBody>
					...
					<p className="text-text-primary">
						{isCursor
							? "This removes the stored Cursor credential snapshot from jacked. Sign in to Cursor IDE again and re-import to add it back."
							: "This removes the stored credential from jacked. Your Claude subscription is untouched, and you can add the account again with OAuth."}
					</p>
				</AlertDialogBody>
				<AlertDialogFooter>
