...
import type { RuntimeJackedInstallationsOverview } from "@/runtime/types";
import { Button } from "@/components/ui/button";
import { MANAGER_LABELS } from "@/jacked/manager-labels";
import { getRuntimeTrpcClient } from "@/runtime/trpc-client";

interface JackedInstallationsViewProps {
	online: boolean;
}
 ...
 */
export function JackedInstallationsView({ online }: JackedInstallationsViewProps): ReactElement {
	const [overview, setOverview] = useState<RuntimeJackedInstallationsOverview | null>(null);
	...
	useEffect(() => {
		...
		const load = async () => {
			...
			setError(null);
			try {
				const result = await getRuntimeTrpcClient(null).jacked.installationsOverview.query();
				if (cancelled) {
					return;
				}
				setOverview(result);
	...
	return (
		<div className="flex min-h-0 flex-1 flex-col overflow-hidden" data-testid="jacked-installations-view">
			<div className="flex items-center gap-1 border-b border-border px-2 py-1 shrink-0">
				<span className="flex-1 text-[10px] text-text-tertiary">
					{overview ? `jacked ${overview.version}` : loading ? "Loading…" : "Installations"}
				</span>
				<Button
					variant="ghost"
					size="sm"
