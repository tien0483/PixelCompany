...
0. **Authorization gate - run this before anything else.** Cloning reproduces someone's design, copy, and assets verbatim, so confirm the use is legitimate before touching the target:
   - Ask the user to confirm one of: (a) they own or operate the target site, (b) they have written permission to reproduce it, or (c) the clone is for private study or prototyping that will not be published or passed off as the original. Do not proceed until they answer.
   - Fetch `<origin>/robots.txt`. If the target path is disallowed for crawling, tell the user and get an explicit go-ahead before continuing; note the finding in docs/research/.
   - Refuse outright if the request is impersonation-shaped: login, checkout, or payment pages of a service the user does not own, bank or government portals, or any stated intent to deceive users about a site's origin. These are not gray areas.
   - Trademarks, logos, and brand assets stay out of published clones unless the user owns them or has permission to use them.
...
3. **Verify or bootstrap the base project.** This skill was designed for a Next.js + shadcn/ui + Tailwind scaffold, but do not assume one exists:
   - If the current repo already builds a frontend (Next.js, Vite/React, SvelteKit, etc.), target it: adapt output paths and component conventions to the existing framework and confirm its build passes before starting.
   - If the repo has no frontend scaffold, create one now instead of sending the user away: confirm the target directory with the user, run `npx create-next-app@latest <dir> --typescript --tailwind --eslint --app`, then `npx shadcn@latest init` inside it, and verify `npm run build` passes before proceeding.
   - If the user points at a different existing project, honor that and adapt to it.
...
*Adapted with credit from [JCodesMore's ai-website-cloner-template](https://github.com/JCodesMore/ai-website-cloner-template) (MIT). jacked additions: the authorization gate (Pre-Flight 0) and the scaffold bootstrap (Pre-Flight 3).*
