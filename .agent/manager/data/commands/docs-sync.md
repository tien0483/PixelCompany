...
> **Note:** If `.claude/commands/docs-sync.md` exists in the current repo, that version has pre-filled repo config from `/manager-setup docs-sync` — use it instead of this global file. If it doesn't exist, continue here.
...
If the config overlay date is more than 90 days old, mention: "Your `/docs-sync` config is over 90 days old — consider running `/manager-setup docs-sync` to refresh it."
