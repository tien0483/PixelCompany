...
> **How it works:** Each generated file is fully standalone — it embeds the complete engine logic with repo-specific config pre-filled. No jacked installation required to USE the generated commands. Commit these files to your repo so collaborators can use them without installing jacked. To get engine updates, upgrade jacked and re-run `/manager-setup`. The engine's `## Config Override` section detects the `## Repo Config` header and skips discovery automatically.
>
> **Living document, not a one-time setup:** an outdated config is worse than a lean one. Each generated file stamps a repo fingerprint (generation commit + manifest set) and carries a self-contained staleness check so it can flag when the codebase has drifted away from the cached config. Re-run `/manager-setup` as the project evolves — see the "When to re-run" triggers in Step 6.
>
...
```
/manager-setup generates repo-specific config files that make jacked commands faster and
...
**Floor check:** If ALL of the following are true — no manifest files found, zero source files detected, and zero git commits — this repo has no useful context to cache. Tell the user: "This repo doesn't have enough structure yet for `/manager-setup` to generate useful config. Add some code and commit history first, then try again." Stop here.
...
---
# Generated by /manager-setup — <today's date> | Template v1 | Engine: jacked v<VERSION>
# Standalone — no dependencies. Commit this file. To update: uv tool install --upgrade claude-jacked && jacked install && /manager-setup <target>
...
3. If EITHER differs, count the intervening commits (`git rev-list --count <generated-commit>..HEAD 2>/dev/null`) and surface one line, then continue:
   > ⚠️ This `/<target>` config was generated N commits ago and the stack/paths may have drifted — re-run `/manager-setup <target>` to refresh.
...
```markdown
---
description: "Roadmap advisor — standalone (generated <date>; upgrade jacked + re-run /manager-setup to update)"
---
# Generated by /manager-setup — <date> | Template v1 | Engine: jacked v<VERSION>
# Standalone — no dependencies. Commit this file. To update: uv tool install --upgrade claude-jacked && jacked install && /manager-setup whats-next
...
- **Access**: none
- **To enable**: install an Asana MCP plugin (recommended) — or set `ASANA_PERSONAL_ACCESS_TOKEN` from https://app.asana.com/0/my-apps — or install an `asana` CLI. Then re-run `/manager-setup whats-next`.
...
```markdown
---
description: "Browser QA — standalone (generated <date>; upgrade jacked + re-run /manager-setup to update)"
---
# Generated by /manager-setup — <date> | Template v1 | Engine: jacked v<VERSION>
# Standalone — no dependencies. Commit this file. To update: uv tool install --upgrade claude-jacked && jacked install && /manager-setup qa
...
```markdown
---
description: "Parallel UX checks — standalone (generated <date>; upgrade jacked + re-run /manager-setup to update)"
---
# Generated by /manager-setup — <date> | Template v1 | Engine: jacked v<VERSION>
# Standalone — no dependencies. Commit this file. To update: uv tool install --upgrade claude-jacked && jacked install && /manager-setup ux
...
```markdown
---
description: "Parallel recursive review — standalone (generated <date>; upgrade jacked + re-run /manager-setup to update)"
---
# Generated by /manager-setup — <date> | Template v1 | Engine: jacked v<VERSION>
# Standalone — no dependencies. Commit this file. To update: uv tool install --upgrade claude-jacked && jacked install && /manager-setup dcr
...
```markdown
---
description: "Docs sync — standalone (generated <date>; upgrade jacked + re-run /manager-setup to update)"
---
# Generated by /manager-setup — <date> | Template v1 | Engine: jacked v<VERSION>
# Standalone — no dependencies. Commit this file. To update: uv tool install --upgrade claude-jacked && jacked install && /manager-setup docs-sync
...
```markdown
---
description: "Release manager — standalone (generated <date>; upgrade jacked + re-run /manager-setup to update)"
---
# Generated by /manager-setup — <date> | Template v1 | Engine: jacked v<VERSION>
# Standalone — no dependencies. Commit this file. To update: uv tool install --upgrade claude-jacked && jacked install && /manager-setup release
...
To pick up future engine improvements: `uv tool install --upgrade claude-jacked && jacked install` then re-run `/manager-setup <target>`.
```
...
If the repo is greenfield (<10 commits), add: "This is a young repo — re-run `/manager-setup <target>` as your project matures to capture new planning docs and lifecycle changes."
...
**When to re-run:** the generated config is a living document — re-run `/manager-setup <target>` when:
