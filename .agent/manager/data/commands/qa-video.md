...
Per the jacked artifact rule (see `qa.md` / `~/.claude/manager-reference.md` § Artifact Format), write this **as HTML, not Markdown**. Copy `~/.claude/jacked-templates/plan-template.html` and save to `docs/superpowers/qa/{YYYY-MM-DD}-qa-video-journey.html` (or to the scratchpad if the user wants nothing in the repo).
