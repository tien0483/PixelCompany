...
DB_PATH = Path.home() / ".claude" / "manager.db"
_VALID_TABLES = {
    "command_usage",
    "agent_invocations",
    "hook_executions",
...
def _already_installed() -> bool:
    ...
    return (_manager_home() / ".claude" / "jacked-manifest.json").exists()
...
def recover(cwd, exclude, session_id, as_digest, limit, depth, budget, as_json):
    ...
    from datetime import datetime, timezone
    from manager import recover as rec
...
def webux(host: str | None, port: int, no_browser: bool, reload: bool):
    ...
    from manager.service.bind import create_sockets, resolve_bind, set_active_plan
...
def _spawn_service_detached(host: str | None, port: int):
    ...
    windowless pythonw.exe path and CREATE_NO_WINDOW for the manager.exe fallback
    ...
    from manager.findbin import find_bin
    ...
    if sys.platform == "win32":
        # ROOT CAUSE of "close the window and the tray dies": the uv `manager.exe`
        ...
        #
        # Fix: launch the GUI-subsystem `pythonw.exe -m manager` instead. pythonw
        # never gets a console, so the service is truly windowless and outlives
        # the launching terminal. Fall back to manager.exe if pythonw isn't found.
        ...
        if pythonw.exists():
            ...
            # manager.exe is the console trampoline: under DETACHED_PROCESS it
...
def start(host: str | None, port: int | None, restart: bool):
    ...
    """
    from manager.service import DEFAULT_PORT, PID_FILE
    from manager.service.process import (
        is_port_available,
        is_process_alive,
        read_pid,
        remove_pid,
...
def check_version():
    ...
    from manager.version_check import check_version_cached
...
def upgrade(extras: str, skip_service: bool):
    ...
    from manager import __version__
    from manager.findbin import find_bin
    from manager.install_method import (
        can_auto_upgrade,
        detect_install_method,
        upgrade_command,
        upgrade_command_label,
    )
    from manager.service import DEFAULT_HOST, DEFAULT_PORT, PID_FILE
...
def _spawn_windows_upgrade_helper(
    ...
    Running manager.exe can't be overwritten while we're holding it open.
    ...
    console.print(
        ...
        "This process will now exit so `manager.exe` can be replaced."
    )
    ...
    # Exit immediately so the lock on manager.exe releases.
    sys.exit(0)
...
def _valid_hook_names() -> frozenset[str]:
    ...
    """
    hooks_dir = _get_runtime_data_root() / "hooks"
...
def _update_status_init_shim(from_version, to_version, method, log_path):
    ...
    """
    from manager.service import update_status as us_mod
    try:
...
def _hook_shim(name: str, hook_args: tuple):
    ...
    import importlib
    try:
        module = importlib.import_module(f"manager.hooks.{name}")
...
def _get_data_root() -> Path:
    ...
    from manager.data_paths import get_catalog_data_root

    return get_catalog_data_root()


def _get_runtime_data_root() -> Path:
    """Runtime root for hooks, web, lenses, guardrails, git-hooks, templates."""
...
def _get_package_data_root() -> Path:
    """Backward-compatible alias for :func:`_get_runtime_data_root`."""
    return _get_runtime_data_root()
...
def _is_editable_install() -> bool:
    ...
    True
    """
    from manager.data_paths import _pixeloffice_repo_root

    repo_root = _pixeloffice_repo_root(Path(__file__).resolve().parent)
    if repo_root is not None and (repo_root / ".git").is_dir():
        return True
    package_root = Path(__file__).resolve().parent.parent
    return (package_root / ".git").is_dir()


def _manager_home() -> Path:
    """Resolve Manager home dir for manifest/last-install/asset install.

    Honors ``$MANAGER_HOME`` (``$JACKED_HOME`` legacy fallback) so tests can
    redirect the ~/.claude tree; defaults to the real home directory.
    """
    import os as _os

    return Path(_os.getenv("MANAGER_HOME") or _os.getenv("JACKED_HOME") or Path.home())


_jacked_home = _manager_home
...
_JACKED_HOOK_PATH_MARKERS = (
    ...
    "-m manager _hook ",                     # fallback form (dev without PATH shim)
)


def _is_jacked_managed_hook_path(command: str) -> bool:
    """True if this settings.json command value was installed by manager.
...
def _build_hook_command(hook_name: str) -> str:
    ...
    binary path). Falls back to `{python} -m manager _hook <name>` when
    ...
    return f'"{python_exe}" -m manager _hook {hook_name}'
...
def _get_behavioral_rules() -> str:
    ...
    rules_path = _get_data_root() / "rules" / "manager_behaviors.md"
...
def _remove_security_hook(settings_path: Path) -> bool:
    ...
    if modified:
        ...
        prompt_path = _manager_home() / ".claude" / "gatekeeper-prompt.txt"
...
def _install_memory_capture_hook(existing: dict, settings_path: Path):
    ...
    ``manager.memory.hooks_config`` so the CLI and the dashboard Features route
    ...
    from manager.memory import hooks_config
...
def _rewire_memory_hooks_on_install(home: Path, existing: dict, settings_path: Path) -> None:
    ...
    silent no-op.
    """
    from manager.memory import vault as _mem_vault
...
def _remove_memory_hooks(settings_path: Path) -> bool:
    ...
    Marker-scoped via ``manager.memory.hooks_config`` (anchored on the
...
def _install_statusline(home: Path, existing: dict, settings_path: Path) -> None:
    ...
    """
    from manager import statusline_setup
...
def _remove_statusline(settings_path: Path) -> bool:
    ...
    Only a jacked-owned command (marker: ``-m manager.statusline``) is
    ...
    changed = statusline_setup.remove_on_uninstall(_manager_home(), settings)
    if changed:
        _write_settings_atomic(settings_path, settings)
...
def _run_claude_mcp(
    *args: str, timeout: int = 10
    ...
    from manager.winproc import NO_WINDOW
...
def install(
    ...
    if packs_csv:
        from manager import packs as _packs_val
    ...
    from manager import __version__ as _ver
    from manager import install_manifest as _mani
    from manager import install_summary as _isum
    ...
    # user-edited SIDECAR from manager's own content (SKILL.md alone can't).
    ...
    if not no_codex:
        try:
            from manager.codex import installer as _cdx
...
def _run_install(
    *,
    ...
    runtime_root = _get_runtime_data_root()

    if not as_json:
        console.print("[bold]Installing manager...[/bold]\n")
    ...
    if skills_src_dir.exists():
        ...
        from manager import install_manifest as _skill_mani
    ...
    ref_src = pkg_root / "rules" / "manager-reference.md"
    ref_dst = home / ".claude" / "manager-reference.md"
    if ref_src.exists():
    ...
    lenses_src = runtime_root / "lenses"
    ...
    # checkpoints). The format preference rule in manager_behaviors.md points
    # Claude here as the starting point for any human-consumed artifact.
    templates_src = runtime_root / "templates"
    ...
    # Statusline: ON by default. Registers `"<abs-python>" -m manager.statusline`
    ...
    from manager.guardrails import deploy_templates
...
def _run_packs_phase(
    ...
    from manager import packs as _packs
...
def _ensure_autostart_and_running(
    ...
    """
    from manager.service import PID_FILE
    from manager.service.platform import install_autostart
    from manager.service.process import is_process_alive, read_pid
...
def _setup_tray_autostart() -> None:
    ...
    if _is_headless():
        ...
        return
    from manager.service import DEFAULT_PORT
...
def _installed_plugins() -> set[str]:
    ...
    settings_path = _manager_home() / ".claude" / "settings.json"
    if settings_path.exists():
        try:
            settings = json.loads(settings_path.read_text(encoding="utf-8"))
...
def doctor():
    ...
    from manager import __version__
    from manager.install_method import detect_install_method
    from manager.service import DEFAULT_HOST, DEFAULT_PORT, PID_FILE
    from manager.service.process import (
    ...
    # Plist/unit check
    if sys.platform == "darwin":
        from manager.service.platform import _get_launchd_plist_path
        ...
        from manager.service.platform import _get_systemd_user_unit_path
...
def uninstall(yes: bool, sounds: bool, security: bool, rules: bool):
    ...
    console.print("[bold]Uninstalling manager...[/bold]\n")
    ...
    ref_path = home / ".claude" / "manager-reference.md"
    if ref_path.exists():
        ref_path.unlink()
...
def memory_group():
    ...
    try:
        from manager.memory import vault as _vault
...
def memory_init(roots: tuple, yes: bool):
    ...
    from manager import memory as _memory
...
def _memory_finish_enable(home) -> None:
    ...
    """
    from manager.memory import setup as _setup
    from manager.memory.settings_io import SettingsUnreadableError
...
def memory_status(quiet: bool):
    ...
    if st["enabled"]:
        try:
            from manager.memory import hooks_config, settings_io
    ...
    if st["initialized"]:
        try:
            ...
            from manager.memory import migrate as _migrate
...
def memory_capture_merge(repo: str):
    ...
    """
    try:
        from manager.memory import merge_capture as _merge_capture
...
def memory_rollup():
    ...
    from manager import memory as _memory
    from manager.memory import rollup as _rollup
...
def _persist_remote_access_from_host(host: str) -> bool:
    ...
    failed).
    """
    from manager.service.migrate import map_host_to_setting
...
def _resolve_service_start_host(typed_host: str | None) -> str | None:
    ...
    try:
        if sys.platform == "darwin":
            ...
            from manager.service.platform import _get_windows_startup_path
        ...
        from manager.service.migrate import (
            extract_baked_host,
...
def service_stop():
    ...
    """
    from manager.service import PID_FILE
    from manager.service.process import stop_process_graceful
...
def service_restart(host: str | None, port: int | None, foreground: bool):
    ...
    """
    from manager.service import DEFAULT_PORT, PID_FILE
    from manager.service.platform import ensure_native_lifecycle, native_restart
    from manager.service.process import (
        stop_process_graceful,
        wait_for_port_free,
    )
...
def service_status():
    ...
    from manager.service import PID_FILE
    from manager.service.process import read_pid, is_process_alive
    from manager.service.platform import detect_autostart
    ...
    if info and is_process_alive(info["pid"]):
        ...
        console.print(f"  Autostart: {autostart_label}")
        if info:
            from manager.service.process import remove_pid
            remove_pid(PID_FILE)
...
def service_uninstall():
    ...
    from manager.service.platform import uninstall_autostart
...
def guardrails_init(repo: str, language: str, force: bool):
    ...
    """
    from manager.guardrails import create_guardrails
...
def lint_hook_init(repo: str, language: str, force: bool):
    ...
    """
    from manager.guardrails import install_hook
...
def claude_cmd(account, claude_args):
    ...
    """
    from manager.launch import launch_claude, prepare_account_dir, resolve_account
    from manager.web.database import Database
...
def _usage_row(acct: dict, now) -> dict:
    ...
    """
    from manager.service.usage_pacing import cache_age_seconds, coerce_pct
...
def codex_add_cmd(make_active, no_login):
    """Add (or refresh) the logged-in Codex account in manager.
...
def _codex_db():
    ...
    return Database(str(Path.home() / ".claude" / "manager.db"))
...
def codex_launch_cmd(account_id, codex_args):
    ...
    """
    from manager.codex.switching import CodexSwapError, launch_codex
...
def init_project(repo: str, language: str, force: bool):
    ...
    """
    from manager.guardrails import create_guardrails, install_hook
