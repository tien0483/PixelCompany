...
The installer registers it as `"<abs-python>" -m manager.statusline` with
...
Hard constraints:
- stdlib only, and never import manager.cli (click + rich cost ~50ms per
...
def main() -> int:
    ...
    try:
        if sys.platform == "win32":
            ...
            # middle dot; replace rather than die (same guard as manager.cli).
            try:
