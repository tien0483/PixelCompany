...
from manager.service import CLAUDE_DIR, DEFAULT_PORT, LAUNCHD_LABEL
...
def _get_systemd_user_unit_path() -> Path:
    ...
    return Path.home() / ".config" / "systemd" / "user" / "manager.service"
...
def native_restart() -> tuple[bool, str]:
    ...
    Per-platform behavior:
      - macOS:   if ai.hank.manager.plist is installed, `launchctl kickstart -k`.
...
def _generate_windows_vbs(
    ...
    Prefer the GUI-subsystem ``pythonw.exe -m manager`` (never touches a console)
    over the ``manager.exe`` console trampoline — same reasoning as
    ...
    if pythonw.exists():
        return (
            ...
            f'WshShell.Run """{pythonw}"" -m manager service start'
            f' --port {port}", 0, False\n'
        )
    return (
        'Set WshShell = CreateObject("WScript.Shell")\n'
...
def _migrate_artifact_host(path: Path, kind: str) -> None:
    ...
    """
    from manager.service.migrate import extract_baked_host, migrate_baked_host_to_db
