...
Run via `python -m manager.service.updater <parent_pid> [extras]`.
...
from manager.service.process import is_process_alive, is_port_available
...
def _spawn_detached(cmd: list, log_fh=None) -> "subprocess.Popen":
    ...
    if sys.platform == "win32":
        # CREATE_NO_WINDOW, not DETACHED_PROCESS: callers hand us the manager.exe
...
def run_update(
    ...
    from manager import __version__ as _current_version
    ...
    try:
        ...
        if not _restart_attempted[0]:
            try:
                from manager.service.platform import native_restart
...
def _find_updater_python() -> str | None:
    ...
    Must be an interpreter that can `import manager.service.updater` —
...
def spawn_updater_from_tray(
    ...
    _spawn_detached(
        [
            py, "-m", "manager.service.updater", str(parent_pid), extras,
...
def _cli() -> None:
    """Entry point for `python -m manager.service.updater <pid> [extras] [--target-version V] [--port P]`."""
    import argparse
    ap = argparse.ArgumentParser(prog="python -m manager.service.updater")
