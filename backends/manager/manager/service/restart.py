...
def restart_service_now() -> None:
    ...
    X, which is the documented CLI-override precedence; see :mod:`manager.service.bind`.)
    """
...
def _exec_target() -> tuple[str, list[str]]:
    ...
    (``jacked webux``), but under ``python -m manager`` it is the package's
    ...
    bind. In that case re-exec through the interpreter with ``-m manager``.
    """
    argv0 = sys.argv[0]
