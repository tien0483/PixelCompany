...
Design contracts:
- The registered command is `"<abs-python>" -m manager.statusline` with
...
from manager.memory.settings_io import read_settings, settings_path, write_settings

STATE_PATH_NAME = "jacked-statusline.json"
STATE_VERSION = 1

# Substring that marks a statusLine command as jacked-managed. Both the
# current form ("<python>" -m manager.statusline) and any future wrapper
# must keep it, or uninstall would strand the entry.
COMMAND_MARKER = "-m manager.statusline"
...
def build_command() -> str:
    ...
    """
    return f'"{sys.executable}" -m manager.statusline'


def is_ours(command: object) -> bool:
    """True when a statusLine command string is jacked-managed.

    >>> is_ours('"/usr/bin/python3" -m manager.statusline')
    True
