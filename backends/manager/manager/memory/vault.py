...
MEMORY_LOGGER_NAME = "manager.memory"
...
_GIT_FALLBACK_EMAIL = "memory@manager.local"
_GIT_TIMEOUT = 30
...
# --------------------------------------------------------------------------- #
# Durable failure logging (rotating file handler on the manager.memory logger)
# --------------------------------------------------------------------------- #
...
def ensure_memory_file_logging(home: Path | None = None) -> None:
    """Attach the rotating memory failure log to ``manager.memory`` exactly once.
    ...
    from every ``manager.memory.*`` module lands in ``<home>/.claude/jacked-memory.log``.
    """
