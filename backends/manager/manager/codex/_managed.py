...
def _mcp_block_body() -> str:
    ...
    from manager.cli import CHROME_DEVTOOLS_MODES, CHROME_DEVTOOLS_NPX_PACKAGE
...
# (present in both the `"jacked" _hook <name>` shim and the `-m manager _hook
...
def _codex_qa_hook_command() -> str:
    ...
    <name>` shim / `"{python}" -m manager _hook <name>` fallback the Claude side
