...
def reset_locks() -> None:
    ...
    See manager.api.routes.auth.reset_locks for the full explanation of
...
def _same_tier_advisory_applies(
    *,
    ...
    """
    from manager.web.auto_swap import TIER_T0, TIER_T1
...
def _compute_poll_interval(
    ...
    if active_id is None or db is None:
        return 60.0, "unknown"
    try:
        from manager.web.auth import compute_urgency_tier, _get_usage_state, _TIER_INTERVALS
...
def _clamp_poll_interval(
    ...
    from manager.web.auto_swap.selection import _FIVE_H_HEADROOM_LIMIT
...
def _candidate_staleness_override(acct: dict) -> bool:
    ...
    """
    from manager.web.window_keeper import needs_7d_ping
...
async def _execute_swap(
    ...
    from manager.api.credential_helpers import invalidate_live_cred_cache
    from manager.web.auto_swap import format_account_label
...
async def active_account_poll_loop(app):
    ...
    values are defined ONCE in manager.web.auth (compute_urgency_tier +
    ...
    while True:
        try:
            ...
            if not _initial_fetch_done:
                from manager.web.auth import fetch_usage as _prime_fetch
            ...
            if _ws and active_acct:
                ...
                try:
                    from manager.service.menubar_summary import binding_model_compact
...
async def full_sweep_loop(app):
    ...
    while True:
        ...
        try:
            ...
            # -- Late imports --------------------------------------------
            from manager.web.auth import fetch_usage
            from manager.web.window_keeper import (
                is_active_hours,
                is_prewake_time,
                needs_ping,
                needs_7d_ping,
