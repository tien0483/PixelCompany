...
def _read_settings_json() -> dict:
    ...
    """
    from manager.memory.settings_io import read_settings
...
def _detect_hook_installed(settings: dict, hook_name: str) -> bool:
    ...
    if hook_name == "memory_vault":
        ...
        from manager.memory import vault as memory_vault_mod
...
async def _toggle_memory_vault(enabled: bool):
    ...
    the engine performs.
    """
    from manager.memory import setup as memory_setup
    from manager.memory import vault as memory_vault
...
async def _toggle_rules(enabled: bool, claude_dir: Path | None = None):
    ...
    if enabled:
        ...
        if not rules_src.exists():
            return JSONResponse(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                content={"error": {"message": "Rules source not found. Reinstall manager.", "code": "SOURCE_UNAVAILABLE"}},
            )
...
def _enable_sound_hooks(settings: dict):
    ...
    from manager.cli import _get_sound_command, _replace_stale_sound_hook, _sound_hook_marker
...
def _disable_sound_hooks(settings: dict):
    ...
    from manager.cli import _sound_hook_marker
...
async def get_chrome_devtools_mcp():
    ...
    from manager.cli import _get_chrome_devtools_mcp_status
...
async def set_chrome_devtools_mcp(body: ChromeDevToolsMCPRequest):
    ...
    from manager.cli import _set_chrome_devtools_mcp_mode
...
async def remove_chrome_devtools_mcp():
    ...
    from manager.cli import _remove_chrome_devtools_mcp
