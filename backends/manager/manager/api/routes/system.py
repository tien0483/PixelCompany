...
from manager.install_summary import DEFAULT_LAST_INSTALL_PATH as _LAST_INSTALL_PATH
from manager.service import update_status as _update_status_mod
...
async def update_project_env(body: EnvUpdateRequest, request: Request):
    ...
    """
    from manager.cli import _validate_env_path, _write_project_env
...
async def detect_project_env(body: ProjectInitRequest, request: Request):
    ...
    """
    from manager.cli import _detect_project_env, _validate_env_path, _write_project_env
...
async def installations_overview(request: Request, repo_path: str | None = None):
    ...
    if db is not None:
        try:
            ...
            from manager.guardrails import check_project_setup
...
async def _do_upgrade(ws_registry) -> None:
    async with _upgrade_lock:
        try:
            ...
            # os.execv(sys.argv[0], ...): under `python -m manager` / `pythonw
            # -m manager` argv[0] is __main__.py (no exec bit) and a raw execv
            # raises OSError, stranding the dashboard on the old code.
            from manager.service.restart import _exec_target
