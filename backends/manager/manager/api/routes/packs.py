...
installed live through the ``npx skills`` CLI (see ``manager.packs``). These
...
verification and npx handling lives in ``manager.packs``; this module only
adapts it to HTTP.
...
def _install_in_thread(pack: packs.Pack, home: Path) -> packs.PackOpResult:
    ...
    """
    try:
        from manager.codex.installer import codex_present
