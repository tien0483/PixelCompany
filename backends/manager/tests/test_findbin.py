...
def test_finds_via_shutil_which():
    ...
    with patch("manager.findbin.shutil.which", return_value="/usr/bin/uv"):
...
def test_falls_back_to_local_bin(fake_bin, tmp_path):
    ...
    with patch("manager.findbin.shutil.which", return_value=None), \
         patch("manager.findbin._home_dir", return_value=str(tmp_path)):
        result = find_bin("uv")
...
def test_respects_uv_tool_bin_dir(fake_bin, tmp_path):
    ...
    target_name = "manager.exe" if sys.platform == "win32" else "jacked"
    ...
    with patch("manager.findbin.shutil.which", return_value=None), \
         ...
         patch("manager.findbin._home_dir", return_value="/nonexistent"):
