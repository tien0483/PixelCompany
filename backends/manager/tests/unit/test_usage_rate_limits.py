...
from manager.web.auth import TokenExchangeResult, fetch_usage
...
class TestRetryPacing:
    ...
    def test_401_retry_failure_does_not_unpace(self):
        ...
        with patch("manager.web.auth.httpx.AsyncClient", return_value=client), \
             patch("manager.web.auth._try_refresh_primary_token",
                   AsyncMock(return_value="fresh-tok")):
            result = asyncio.run(fetch_usage(1, db))
...
class TestRotationThrottle:
    ...
    def test_second_429_within_interval_skips_rotation(self):
        ...
        def run_429():
            ...
            with patch("manager.web.auth.httpx.AsyncClient", return_value=client), \
                 patch("manager.web.auth._refresh_token_flow", flow_mock):
