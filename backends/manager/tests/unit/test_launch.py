...
class TestSeedWorkspaceTrust:
    def test_seeds_trust_from_global(self, tmp_path):
        ...
        from manager.launch import _seed_workspace_trust
...
class TestSeedOauthAccount:
    def test_seeds_when_absent(self, tmp_path):
        ...
        from manager.launch import _seed_oauth_account
...
class TestPrepareAccountDir:
    def test_creates_cred_file(self, tmp_path):
        ...
        with mock.patch("manager.launch.ACCOUNTS_DIR", tmp_path / "accounts"):
            with mock.patch("manager.launch.should_refresh", return_value=False):
    ...
    def test_refreshes_if_near_expiry(self, tmp_path):
        ...
        with mock.patch("manager.launch.ACCOUNTS_DIR", tmp_path / "accounts"):
            with mock.patch("manager.launch.should_refresh", return_value=True):
                ...
                with mock.patch(
                    "manager.web.auth.refresh_account_token", new=mock.MagicMock()
                ):
                    with mock.patch("manager.launch.asyncio") as mock_asyncio:
                        from manager.launch import prepare_account_dir
    ...
    def test_rejects_symlink_dir(self, tmp_path):
        ...
        with mock.patch("manager.launch.ACCOUNTS_DIR", accounts_dir):
            with mock.patch("manager.launch.should_refresh", return_value=False):
                from manager.launch import prepare_account_dir
    ...
    def test_warns_on_keychain_write_failure(self, tmp_path):
        ...
        with mock.patch("manager.launch.ACCOUNTS_DIR", tmp_path / "accounts"):
            with mock.patch("manager.launch.should_refresh", return_value=False):
                with mock.patch("manager.launch.write_platform_credentials", return_value=False):
                    with mock.patch("manager.launch.logger") as mock_logger:
                        from manager.launch import prepare_account_dir
...
class TestResolveAccount:
    def test_with_id(self, tmp_path):
        db = _make_db(tmp_path)
        from manager.launch import resolve_account

        with mock.patch("manager.launch.find_bin", return_value="/usr/local/bin/claude"):
    ...
    def test_no_claude_raises(self, tmp_path):
        ...
        with mock.patch("manager.launch.find_bin", return_value=None):
    ...
    def test_without_id_db_fallback(self, tmp_path):
        ...
        with mock.patch("manager.launch.find_bin", return_value="/usr/local/bin/claude"):
            with mock.patch.object(Path, "home", return_value=tmp_path):
                with mock.patch(
                    "manager.launch.read_platform_credentials", return_value=None
                ):
                    result = resolve_account(None, db)
        assert result["email"] == "alice@test.com"
    ...
    def test_without_id_keychain_stamp_fallback(self, tmp_path):
        ...
        with mock.patch("manager.launch.find_bin", return_value="/usr/local/bin/claude"):
            with mock.patch.object(Path, "home", return_value=tmp_path):
                with mock.patch(
                    "manager.launch.read_platform_credentials", return_value=kc_data
                ):
                    result = resolve_account(None, db)
        assert result["email"] == "bob@test.com"
...
class TestLaunchClaude:
    def test_sets_env_and_launches(self, tmp_path):
        ...
        from manager.launch import launch_claude
        ...
        with mock.patch("manager.launch.subprocess.Popen", return_value=mock_proc) as mock_popen:
...
class TestTokenSync:
    def test_sync_tokens_from_file_updates_db(self, tmp_path):
        ...
        from manager.launch import _sync_tokens_from_file
    ...
    def test_close_sessions_by_pid(self, tmp_path):
        ...
        from manager.launch import _close_sessions_by_pid
...
class TestHookConfigDir:
    def test_get_cred_data_reads_config_dir(self, tmp_path):
        ...
        from manager.hooks.session_account_tracker import _get_cred_data
    ...
    def test_match_uses_path_based_account_id(self, tmp_path):
        ...
        from manager.hooks.session_account_tracker import (
        ...
        with mock.patch(
            "manager.hooks.session_account_tracker.DB_PATH",
    ...
    def test_match_falls_through_for_non_account_dir(self, tmp_path):
        ...
        from manager.hooks.session_account_tracker import _match_token_to_account
...
class TestSessionHookEarlyReturn:
    ...
    def test_handle_event_matches_without_token(self, tmp_path):
        ...
        db = _make_db(tmp_path)
        from manager.hooks.session_account_tracker import _handle_event
        ...
        with mock.patch(
            ...
            with mock.patch(
                "manager.hooks.session_account_tracker._get_cred_data",
                return_value=(None, cred_data),
            ):
                with mock.patch(
                    "manager.hooks.session_account_tracker.CLAUDE_CONFIG",
...
class TestSharedSymlinks:
    ...
    def test_creates_symlinks_for_shared_resources(self, tmp_path):
        ...
        with mock.patch("manager.launch.ACCOUNTS_DIR", tmp_path / ".claude" / "accounts"):
            with mock.patch("manager.launch.should_refresh", return_value=False):
                with mock.patch.object(Path, "home", return_value=tmp_path):
                    from manager.launch import prepare_account_dir
    ...
    def test_replaces_real_file_with_symlink(self, tmp_path):
        ...
        with mock.patch("manager.launch.ACCOUNTS_DIR", claude_dir / "accounts"):
            with mock.patch("manager.launch.should_refresh", return_value=False):
                with mock.patch.object(Path, "home", return_value=tmp_path):
                    from manager.launch import prepare_account_dir
...
class TestClaudeCmdFlagPassthrough:
    ...
    def _make_claude_db(tmp_path, active_account_id="1"):
        """Create DB at the path claude_cmd expects (~/.claude/manager.db)."""
        claude_dir = tmp_path / ".claude"
        claude_dir.mkdir(parents=True, exist_ok=True)
        db = Database(str(claude_dir / "manager.db"))
        with db._writer() as conn:
            conn.execute(
                """INSERT INTO accounts
                   (id, email, access_token, refresh_token, expires_at,
    ...
    def test_resume_flag_passed_through(self, tmp_path):
        ...
        with mock.patch.object(Path, "home", return_value=tmp_path), \
             mock.patch("manager.launch.resolve_account") as mock_resolve, \
             mock.patch("manager.launch.prepare_account_dir") as mock_prepare, \
             mock.patch("manager.launch.launch_claude") as mock_launch:
