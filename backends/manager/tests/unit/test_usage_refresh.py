...
from manager.web.auth import USAGE_CACHE_FRESHNESS_SECONDS, fetch_usage
...
class TestUsageCeiling:
    ...
    def test_429_sets_backoff_in_state(self):
        ...
        with patch("manager.web.auth.httpx.AsyncClient", return_value=client), \
             patch("manager.web.auth._try_refresh_on_429", AsyncMock(return_value=None)):
            result = asyncio.run(fetch_usage(1, db))
...
class TestUrgencyTier:
    ...
    def test_idle_low_usage_no_burn(self):
        ...
        from manager.web.auth import compute_urgency_tier
...
class TestEscalatingBackoff:
    def test_consecutive_429s_escalate(self):
        ...
        state["last_fetched_at"] = time.time() - 200
        with patch("manager.web.auth.httpx.AsyncClient", return_value=client), \
             patch("manager.web.auth._try_refresh_on_429", refresh_mock):
            asyncio.run(fetch_usage(1, db))
