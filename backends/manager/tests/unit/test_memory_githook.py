...
Two surfaces:
  - manager.memory.githook       per-repo post-merge hook install/remove
  - manager.memory.merge_capture the ORIG_HEAD..HEAD distillation engine
...
from manager.memory import capture, githook, merge_capture
...
def test_cli_capture_merge_exits_zero_when_everything_raises(env, monkeypatch):
    ...
    monkeypatch.setattr("manager.memory.merge_capture.capture_merge", boom)
