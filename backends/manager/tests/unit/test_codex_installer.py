...
from manager.codex import installer as ins
...
def data_root(tmp_path):
    ...
    (root / "rules" / "manager_behaviors.md").write_text("# manager behaviors\nbe blunt\n")
    return root
...
def test_install_excludes_claude_only_commands(data_root, homes):
    """swarm / goal-maker / browser-reset / manager-setup are wired to Claude Code
    ...
    for name in ("swarm.md", "goal-maker.md", "browser-reset.md", "manager-setup.md"):
...
def test_excluded_commands_produce_no_skill(data_root, homes):
    ...
    _install(data_root, homes)
    manifest = _manifest(homes)
    for stem in ("swarm", "goal-maker", "browser-reset", "manager-setup"):
...
_RULES_WITH_CLAUDE = (
    ...
    "- read ~/.claude/manager-reference.md for details.\n"
)


def _real_rules_text():
    return (CATALOG_ROOT / "rules" / "manager_behaviors.md").read_text()
...
def test_codex_rules_body_rewrites_claude_md():
    ...
    assert "~/.claude/manager-reference.md" in source     # lowercase path preserved
...
def test_codex_rules_body_real_data_guard():
    """Against the REAL manager_behaviors.md: the rewritten source body carries no
...
def _install_with_claude_rules(data_root, homes):
    (data_root / "rules" / "manager_behaviors.md").write_text(_RULES_WITH_CLAUDE)
...
def test_qa_hook_command_uses_build_hook_command_shim(data_root, homes):
    ...
    from manager.cli import _build_hook_command
...
def test_mutable_files_written_atomically(data_root, homes, monkeypatch):
    ...
    # The atomic writer moved to manager.codex._fsutil and is imported into both
    ...
    monkeypatch.setattr("manager.codex._managed._atomic_write_text", spy)
    _install(data_root, homes)
