...
# Repo root, so a spawned `python -c` subprocess resolves the editable manager.
...
def _fixture_root(env):
    ...
    _make_repo(root / "claude-jacked", "git@github.com:jackneil/claude-manager.git")
    return root
...
@pytest.mark.parametrize("url,expected", [
    ("git@github.com:jackneil/claude-manager.git", "github.com/jackneil/claude-jacked"),
...
def test_update_state_concurrent_writers_keep_both(env):
    ...
    prog = textwrap.dedent(
        ...
        from manager.memory import vault as v
        key = sys.argv[2]
        for _ in range(20):
