...
def test_updater_py_covers_all_phases():
    import manager.service.updater as _u_mod
...
def test_windows_batch_body_contains_every_phase():
    ...
    with patch("manager.install_method.detect_install_method", return_value="uv"):
        with patch("manager.service.updater.find_bin", return_value=r"C:\uv\uv.exe"):
...
def test_update_html_still_embeds_every_phase():
    ...
    from tests._runtime import RUNTIME_ROOT

    html = (RUNTIME_ROOT / "web" / "update.html").read_text(encoding="utf-8")
    for name in PHASE_NAMES:
