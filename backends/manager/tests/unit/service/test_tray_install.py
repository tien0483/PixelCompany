...
def _patch_autostart(monkeypatch, *, result, pid=None, alive=True, platform="win32"):
    import manager.cli as cli
    monkeypatch.setattr("manager.service.platform.install_autostart", lambda *a, **k: result)
    monkeypatch.setattr("manager.service.process.read_pid", lambda *a, **k: pid)
    monkeypatch.setattr("manager.service.process.is_process_alive", lambda _pid: alive)
...
class TestEnsureAutostartAndRunning:
    ...
    def test_spawn_failure_is_non_fatal(self, monkeypatch):
        import manager.cli as cli
        monkeypatch.setattr(
            "manager.service.platform.install_autostart",
            lambda *a, **k: "Installed startup script: x",
        )
        monkeypatch.setattr("manager.service.process.read_pid", lambda *a, **k: None)
