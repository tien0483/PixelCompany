"""Tests for manager.service.migrate — baked --host extraction, host-free
...
def _vbs_pythonw(host: str | None, port: int = 8321) -> str:
    ...
    return (
        ...
        f'WshShell.Run """C:\\Py\\pythonw.exe"" -m manager service start'
...
def _vbs_exe(host: str | None, port: int = 8321) -> str:
    ...
    return (
        'Set WshShell = CreateObject("WScript.Shell")\n'
        f'WshShell.Run """C:\\bin\\manager.exe"" service start'
        f'{host_part} --port {port}", 0, False\n'
    )
...
class TestMigrateBakedHostToDb:
    ...
    def test_unmapped_ip_never_writes_and_info_logs(self, db, caplog):
        with caplog.at_level(logging.INFO, logger="manager.service.migrate"):
