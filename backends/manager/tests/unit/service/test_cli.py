...
def _mem_db(monkeypatch):
    ...
    db = Database(":memory:")
    monkeypatch.setattr("manager.web.database.Database", lambda *a, **k: db)
    return db
...
class TestSpawnServiceDetachedArgv:
    ...
    @patch("manager.findbin.find_bin", return_value="/fake/jacked")
    @patch("subprocess.Popen")
    def test_no_host_omits_host_flag(self, mock_popen, _mock_find):
        from manager.cli import _spawn_service_detached
...
class TestWebuxHostResolution:
    ...
    def test_webux_sets_jacked_host_from_plan_and_passes_sockets(self, monkeypatch):
        ...
        from manager.service.bind import BindPlan
        ...
        monkeypatch.setattr("manager.service.bind.resolve_bind", lambda h, p: plan)
        monkeypatch.setattr(
            "manager.service.bind.create_sockets", lambda _plan: ["S1", "S2"]
        )
    ...
    def test_webux_reload_is_single_host_and_skips_resolve_bind(self, monkeypatch):
        ...
        monkeypatch.setattr("manager.service.bind.resolve_bind", _boom)
        monkeypatch.setenv("JACKED_HOST", "sentinel")
...
class TestServiceStatus:
    def test_status_when_not_running(self, tmp_path):
        ...
        with patch("manager.service.PID_FILE", pid_file):
...
class TestServiceStop:
    ...
    @patch(
        "manager.service.process.stop_process_graceful",
...
class TestServiceInstall:
    @patch("manager.cli._spawn_service_detached")
    @patch("manager.service.process.read_pid", return_value=None)
    @patch("manager.service.platform.install_autostart")
...
class TestServiceUninstall:
    @patch("manager.service.platform.uninstall_autostart")
...
class TestServiceRestart:
    ...
    def test_restart_when_not_running_starts_fresh_detached(
        ...
        if _sys.platform == "win32":
            # Windows launches `pythonw.exe -m manager` to dodge the uv
            # console-trampoline window; manager.exe is only the fallback.
    ...
    @patch("manager.findbin.find_bin", return_value="/fake/jacked")
    @patch("subprocess.Popen")
    @patch("manager.service.process.wait_for_port_free", return_value=True)
    @patch(
        "manager.service.process.stop_process_graceful",
        return_value={"was_running": True, "died": True, "killed": False},
    )
    ...
    @patch("manager.service.platform.ensure_native_lifecycle",
           return_value=(False, "unavailable", "test"))
    @patch("subprocess.Popen")
    @patch("manager.service.process.wait_for_port_free", return_value=False)
    @patch(
        "manager.service.process.stop_process_graceful",
        return_value={"was_running": True, "died": True, "killed": False},
    )
    ...
    @patch("manager.service.tray.ServiceRunner")
    @patch(
        "manager.service.process.stop_process_graceful",
        return_value={"was_running": False, "died": False, "killed": False},
    )
...
class TestServiceRestartAutoInstall:
    ...
    @patch("manager.service.platform.native_restart",
    ...
    @patch("manager.service.platform.ensure_native_lifecycle",
           return_value=(False, "unavailable", "no native lifecycle manager"))
    @patch("manager.service.process.stop_process_graceful",
           return_value={"was_running": False, "died": False, "killed": False})
    @patch("subprocess.Popen")
...
class TestStartCommand:
    ...
    def test_noop_when_already_healthy(self):
        from manager.cli import main
        with (
            patch("manager.service.process.read_pid",
                  return_value={"pid": 4242, "port": 8321}),
            patch("manager.service.process.is_process_alive", return_value=True),
            patch("manager.cli._service_http_ok", return_value=True),
            patch("manager.cli._spawn_service_detached") as spawn,
        ):
    ...
    def test_cold_start_when_down(self, tmp_path):
        ...
        with (
            patch("manager.service.process.read_pid", return_value=None),
            patch("manager.service.process.is_process_alive", return_value=False),
            patch("manager.service.process.is_port_available", return_value=True),
            patch("manager.cli._service_http_ok", return_value=False),
            patch("manager.cli._spawn_service_detached",
                  return_value=tmp_path / "svc.log") as spawn,
            patch("manager.cli._wait_service_ready", return_value=True),
        ):
            result = CliRunner().invoke(main, ["start"])
        assert result.exit_code == 0
        assert "running" in result.output.lower()
    ...
    def test_clears_stale_pid_then_starts(self, tmp_path):
        ...
        with (
            ...
            patch("manager.service.process.is_process_alive", return_value=False),
            patch("manager.service.process.is_port_available", return_value=True),
            patch("manager.service.process.remove_pid") as remove,
            patch("manager.cli._service_http_ok", return_value=False),
            patch("manager.cli._spawn_service_detached",
                  return_value=tmp_path / "svc.log") as spawn,
            patch("manager.cli._wait_service_ready", return_value=True),
    ...
    def test_aborts_when_port_held_by_other(self):
        ...
        with (
            patch("manager.service.process.read_pid", return_value=None),
            patch("manager.service.process.is_process_alive", return_value=False),
            patch("manager.service.process.is_port_available", return_value=False),
            patch("manager.cli._service_http_ok", return_value=False),
            patch("manager.cli._spawn_service_detached") as spawn,
        ):
            result = CliRunner().invoke(main, ["start"])
    ...
    def test_exits_nonzero_when_never_ready(self, tmp_path):
        ...
        with (
            ...
            patch("manager.cli._wait_service_ready", return_value=False),
        ):
            result = CliRunner().invoke(main, ["start"])
        assert result.exit_code != 0
...
class TestServiceStartBootMigration:
    ...
    def _invoke(self, monkeypatch, tmp_path, *, baked, argv_host):
        import manager.cli as cli

        monkeypatch.setattr(cli.sys, "platform", "darwin")
        plist = tmp_path / "ai.hank.manager.plist"
        ...
        if argv_host is not None:
            args += ["--host", argv_host]
        with (
            patch("manager.service.platform._get_launchd_plist_path",
                  return_value=plist),
            patch("manager.service.tray.ServiceRunner") as mock_runner_cls,
        ):
