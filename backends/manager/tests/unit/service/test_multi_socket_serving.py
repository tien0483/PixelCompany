...
~/.claude. (The service-dir conftest already isolates ``manager.service.*``
paths, but NOT ``manager.web.database`` / the lifespan's home-derived paths.)
"""
...
def _bind_ephemeral_loopback() -> tuple[socket.socket, int]:
    ...
    matching what manager.service.bind.create_sockets hands uvicorn."""
...
def test_two_prebound_sockets_both_answer_health(_isolate_home):
    ...
    config = uvicorn.Config("manager.api.main:app", log_level="warning")
