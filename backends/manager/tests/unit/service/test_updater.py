...
class TestWaitForExit:
    def test_returns_true_when_process_exits(self):
        from manager.service.updater import wait_for_exit
...
class TestRunUpdate:
    @patch("manager.service.platform.ensure_native_lifecycle",
           return_value=(False, "unavailable", "test: manual spawn"))
    @patch("manager.install_method.can_auto_upgrade", return_value=(True, ""))
    @patch("manager.install_method.detect_install_method", return_value="uv")
    @patch("manager.service.updater.is_port_available", return_value=True)
    @patch("manager.service.updater.find_bin")
    ...
    def test_order_wait_install_migrate_restart(
        ...
        from manager.service import updater
    ...
    def test_finally_guard_kicks_service_when_upgrade_phase_bails_early(
        ...
        with patch("manager.service.platform.native_restart", return_value=(True, "test-kickstart")) as mock_restart, \
...
class TestWindowsTrayUpdaterBatch:
    ...
    @patch("manager.install_method.can_auto_upgrade", return_value=(True, ""))
    @patch("manager.install_method.detect_install_method", return_value="uv")
    @patch("manager.findbin.find_bin", return_value=r"C:\uv\uv.exe")
    @patch("subprocess.Popen")
    def test_tray_update_wait_loop_is_bounded(
        ...
        from manager.service.updater import _spawn_windows_tray_updater
...
class TestPortWaitBeforeServiceStart:
    ...
    @patch("manager.install_method.can_auto_upgrade", return_value=(True, ""))
    @patch("manager.install_method.detect_install_method", return_value="uv")
    @patch("manager.service.updater.time.sleep", lambda _s: None)
    @patch("manager.service.updater.find_bin")
    @patch("manager.service.updater.is_port_available")
    @patch("subprocess.run")
    @patch("subprocess.Popen")
...
class TestParentKillEscalation:
    ...
    @patch("manager.install_method.can_auto_upgrade", return_value=(True, ""))
    @patch("manager.service.updater.is_port_available", return_value=True)
    @patch("manager.service.updater._force_kill_pid")
    @patch("manager.service.updater.find_bin")
    @patch("subprocess.run")
    @patch("subprocess.Popen")
...
class TestPortStuckRecovery:
    ...
    @patch("manager.service.updater.time.sleep", lambda _s: None)
    @patch("manager.install_method.can_auto_upgrade", return_value=(True, ""))
    @patch("manager.service.updater._pids_bound_to_port", return_value=[54321])
    @patch("manager.service.updater._force_kill_pid")
    @patch("manager.service.updater.is_port_available")
    @patch("manager.service.updater.find_bin")
    @patch("subprocess.run")
    ...
    @patch("manager.install_method.can_auto_upgrade", return_value=(True, ""))
    @patch("manager.service.updater.time.sleep", lambda _s: None)
    @patch("manager.service.updater._pids_bound_to_port", return_value=[])
    @patch("manager.service.updater.is_port_available", return_value=False)
    @patch("manager.service.updater.find_bin")
    @patch("subprocess.run")
    @patch("subprocess.Popen")
...
class TestSpawnDetached:
    ...
    def test_posix_sets_start_new_session(self, mock_popen):
        from manager.service.updater import _spawn_detached
...
class TestFindUpdaterPython:
    def test_uses_current_interpreter(self):
        """Helper must run in a Python that can import manager.service.updater —
        that means the tool venv Python (sys.executable), not a system Python
        that wouldn't have jacked on its path."""
        from manager.service.updater import _find_updater_python
    ...
    def test_chosen_interpreter_can_import_updater_module(self):
        ...
        result = subprocess.run(
            [py, "-c", "import manager.service.updater"],
            capture_output=True,
            timeout=10,
        )
        assert result.returncode == 0, (
            f"Chosen Python {py} cannot import manager.service.updater: "
...
class TestSpawnFromTrayWindows:
    ...
    @patch("manager.install_method.can_auto_upgrade", return_value=(True, ""))
    @patch("manager.install_method.detect_install_method", return_value="uv")
    @patch("manager.service.updater.find_bin", return_value="C:\\Users\\x\\.local\\bin\\uv.exe")
    @patch("subprocess.Popen")
    ...
    @patch("manager.install_method.detect_install_method", return_value="pip")
    @patch("subprocess.Popen")
    ...
    @patch("manager.service.updater.find_bin", return_value="C:\\fake\\uv.exe")
    @patch("subprocess.Popen")
    def test_posix_still_uses_python_subprocess(self, mock_popen, mock_find, monkeypatch, tmp_path):
        """POSIX path unchanged — still spawns python -m manager.service.updater."""
        from manager.service import updater
        monkeypatch.setattr(updater, "UPDATE_LOG", tmp_path / "update.log")
        ...
        args = mock_popen.call_args[0][0]
        assert "-m" in args
        assert "manager.service.updater" in args
...
class TestUpdaterWritesStatus:
    ...
    def test_writes_succeeded_status_with_all_phases(
        ...
        from manager.service import updater, update_status as us_mod
        from manager.service.update_phases import PHASE_NAMES
        monkeypatch.setattr(updater, "UPDATE_LOG", tmp_path / "update.log")
    ...
    @patch("manager.install_method.can_auto_upgrade", return_value=(False, "editable — run git pull"))
...
class TestPosixSpawnThreadsTargetVersion:
    @patch("subprocess.Popen")
    @patch("manager.service.updater._find_updater_python", return_value="/fake/python")
...
class TestWindowsBatchPhases:
    @patch("manager.install_method.detect_install_method", return_value="uv")
    @patch("manager.service.updater.find_bin", return_value=r"C:\uv\uv.exe")
    @patch("subprocess.Popen")
...
class TestRunUpdateTerminalStatus:
    @patch("manager.install_method.detect_install_method", return_value="uv")
    @patch("manager.install_method.can_auto_upgrade", return_value=(True, ""))
    @patch("manager.service.updater.find_bin", return_value=None)
...
def test_run_update_handles_upgrade_command_valueerror(tmp_path, monkeypatch):
    ...
    from manager.service import updater, update_status as us_mod
    from manager import install_method
    recovery_path = tmp_path / "recovery.txt"
    monkeypatch.setattr(updater, "UPDATE_LOG", tmp_path / "update.log")
    monkeypatch.setattr(us_mod, "UPDATE_STATUS_FILE", tmp_path / "status.json")
    monkeypatch.setattr(updater, "_write_recovery",
    ...
    # `from manager.install_method import upgrade_command`, so patching the
...
def test_windows_batch_checks_errorlevel_after_status_writes(
    ...
    monkeypatch.setattr(updater, "UPDATE_LOG", tmp_path / "update.log")
    monkeypatch.setattr(subprocess, "DETACHED_PROCESS", 0x8, raising=False)
    with _patch("manager.install_method.detect_install_method", return_value="uv"), \
         _patch("manager.service.updater.find_bin", return_value=r"C:\uv\uv.exe"), \
