"""Tests for manager.service.tray module."""
...
class TestCreateIcon:
    ...
    def test_running_icon_is_64x64(self):
        _skip_if_no_tray()
        from manager.service.tray import create_icon_image
...
class TestGlyphFont:
    ...
    def test_glyph_font_is_requested_size_not_tiny_default(self):
        _skip_if_no_tray()
        from manager.service.tray import _load_glyph_font
...
class TestBuildMenu:
    ...
    def test_menu_has_expected_items(self):
        _skip_if_no_tray()
        from manager.service.tray import build_menu
        def noop():
            return None
        menu = build_menu(
            port=8321,
...
class TestServiceRunner:
    ...
    @patch("manager.service.bind.create_sockets", return_value=[])
    @patch("manager.service.tray.pystray", create=True)
    @patch("manager.service.tray.uvicorn")
    def test_start_uvicorn_thread_is_daemon(self, mock_uvicorn, mock_pystray, mock_socks):
        ...
        with patch("manager.service.bind.resolve_bind", return_value=plan):
    ...
    def test_start_uvicorn_reresolves_and_sets_env_from_plan(
        ...
        monkeypatch.setattr("manager.service.bind.resolve_bind", _fake_resolve)
    ...
    def test_start_uvicorn_cold_start_raises_systemexit_on_bind_conflict(
        ...
        with (
            patch("manager.service.bind.resolve_bind", return_value=plan),
            patch("manager.service.bind.create_sockets",
...
class TestApplyIcon:
    ...
    def test_passes_update_available_true_when_outdated(self):
        _skip_if_no_tray()
        import manager.service.tray as tray_mod
        from manager.service.tray import ServiceRunner
...
class TestCheckDeps:
    ...
    def test_check_tray_deps_raises_when_missing(self):
        from manager.service import tray
...
class TestOnUpdateClick:
    def test_spawns_updater_then_stops(self):
        ...
        runner._version_info = {"latest": "0.42.0", "outdated": True}
        runner._icon = MagicMock()
        with patch("manager.install_method.can_auto_upgrade", return_value=(True, "")):
            with patch("manager.service.updater.spawn_updater_from_tray") as mock_spawn:
    ...
    def test_click_releases_lock_on_spawn_failure(self):
        ...
        runner._version_info = {"latest": "0.42.0", "outdated": True}
        runner._icon = MagicMock()
        with patch("manager.service.updater.spawn_updater_from_tray", side_effect=RuntimeError("boom")):
    ...
    def test_double_click_only_spawns_once(self):
        ...
        with patch("manager.install_method.can_auto_upgrade", return_value=(True, "")):
            with patch("manager.service.updater.spawn_updater_from_tray", side_effect=slow_spawn):
                with patch.object(runner, "_on_stop"):
    ...
    def test_spawns_before_stops(self):
        ...
        with patch("manager.install_method.can_auto_upgrade", return_value=(True, "")):
            with patch(
                "manager.service.updater.spawn_updater_from_tray",
...
class TestOnCheckForUpdates:
    @patch("manager.service.tray.check_version_cached")
...
class TestVersionCheckThread:
    def test_exits_on_stop_event(self):
        ...
        runner = ServiceRunner()
        runner._icon = None
        with patch("manager.service.tray.check_version_cached", return_value=None):
...
class TestOnUpdateClickRefusal:
    @patch("manager.service.update_status.clear_status")
    @patch(
        "manager.install_method.can_auto_upgrade",
        return_value=(False, "editable — run `git pull && uv sync`"),
    )
    @patch("manager.service.updater.spawn_updater_from_tray")
...
class TestOnUpdateClickBreadcrumbs:
    ...
    @patch("manager.service.updater.spawn_updater_from_tray")
    @patch("manager.service.update_status.init_status")
    ...
    def test_breadcrumb_appended_to_update_log(
        ...
        _skip_if_no_tray()
        from manager.service.tray import ServiceRunner
        from manager.service import updater as updater_mod
        runner = ServiceRunner()
        runner._version_info = {"latest": "0.42.0", "outdated": True}
        runner._icon = MagicMock()
    ...
    def test_bootstrap_targets_loopback_regardless_of_host(self, monkeypatch, tmp_path):
        ...
        with patch("manager.install_method.can_auto_upgrade", return_value=(True, "")):
            with patch("webbrowser.open") as mock_wb:
                with patch("manager.service.updater.spawn_updater_from_tray"):
                    with patch.object(runner, "_on_stop"):
                        runner._on_update_click()
    ...
    def test_falls_back_to_http_url_when_template_unreadable(self, monkeypatch, tmp_path):
        ...
        empty_web.mkdir()
        monkeypatch.setattr("manager.service.CLAUDE_DIR", tmp_path)
        monkeypatch.setattr("manager.api.main.WEB_DIR", empty_web)
        runner = ServiceRunner(port=8321)
        runner._version_info = {"latest": "0.42.0", "outdated": True}
        runner._icon = MagicMock()
...
class TestRestartHandlerRegistration:
    ...
    def test_run_registers_during_and_unregisters_after(self, monkeypatch):
        _skip_if_no_tray()
        from manager.service import restart as restart_mod
        from manager.service.tray import ServiceRunner
        ...
        monkeypatch.setattr(
            "manager.service.tray.select_menubar_backend", lambda *a, **k: "pystray"
        )
        monkeypatch.setattr(
            "manager.service.tray.is_port_available", lambda *a, **k: True
        )
        monkeypatch.setattr("manager.service.tray.write_pid", lambda *a, **k: None)
        monkeypatch.setattr("manager.service.tray.remove_pid", lambda *a, **k: None)
        monkeypatch.setattr(
            "manager.service.platform.detect_autostart", lambda: False
        )
