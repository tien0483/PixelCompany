"""Tests for manager.service.platform module."""
...
class TestGenerateLaunchdPlist:
    def test_contains_label(self):
        from manager.service.platform import _generate_launchd_plist
...
class TestGenerateWindowsVbs:
    def test_prefers_pythonw_when_present(self, tmp_path, monkeypatch):
        # pythonw.exe next to the interpreter -> VBS launches it, not manager.exe
        (tmp_path / "pythonw.exe").write_text("")
        monkeypatch.setattr(
            "manager.service.platform.sys.executable", str(tmp_path / "python.exe")
        )
        from manager.service.platform import _generate_windows_vbs
        vbs = _generate_windows_vbs(jacked_bin=r"C:\bin\manager.exe", port=8321)
        assert "pythonw.exe" in vbs
        assert "-m manager service start" in vbs
        assert r"C:\bin\manager.exe" not in vbs

    def test_falls_back_to_jacked_exe_without_pythonw(self, tmp_path, monkeypatch):
        # no pythonw.exe -> fall back to the manager.exe trampoline
        monkeypatch.setattr(
            "manager.service.platform.sys.executable", str(tmp_path / "python.exe")
        )
        from manager.service.platform import _generate_windows_vbs
        vbs = _generate_windows_vbs(jacked_bin=r"C:\bin\manager.exe", port=8321)
        assert r"C:\bin\manager.exe" in vbs
    ...
    def test_custom_port_in_vbs(self):
        from manager.service.platform import _generate_windows_vbs
        vbs = _generate_windows_vbs(jacked_bin=r"C:\bin\manager.exe", port=9000)
...
class TestDetectAutostart:
    ...
    def test_darwin_detects_plist(self, tmp_path):
        ...
        with patch("manager.service.platform._get_launchd_plist_path", return_value=plist):
    ...
    def test_win32_detects_vbs(self, tmp_path):
        vbs = tmp_path / "manager.vbs"
        vbs.write_text("test")
        from manager.service.platform import detect_autostart
        with patch("manager.service.platform._get_windows_startup_path", return_value=vbs):
            assert detect_autostart() is True
...
_LEGACY_VBS = (
    ...
    'WshShell.Run """C:\\bin\\manager.exe"" service start'
    ' --host 0.0.0.0 --port 8321", 0, False\n'
)


class TestInstallAutostart:
    @patch("manager.service.platform._service_is_running", return_value=False)
    @patch("sys.platform", "darwin")
    ...
    def test_darwin_writes_plist_and_bootstraps(
        ...
        with patch("manager.service.platform._get_launchd_plist_path", return_value=plist):
            with patch("manager.findbin.find_bin", return_value="/usr/local/bin/jacked"):
    ...
    @patch("manager.service.platform._service_is_running", return_value=True)
    @patch("sys.platform", "darwin")
    @patch("subprocess.run")
    def test_darwin_running_service_rewrites_file_only(
        ...
        with patch("manager.service.platform._get_launchd_plist_path", return_value=plist):
            with patch("manager.findbin.find_bin", return_value="/usr/local/bin/jacked"):
                with patch(
                    "manager.service.migrate.migrate_baked_host_to_db",
    ...
    def test_win32_writes_vbs(self, tmp_path):
        ...
        from manager.service.platform import install_autostart
        with patch("manager.service.platform._get_windows_startup_path", return_value=vbs):
            with patch("manager.findbin.find_bin", return_value=r"C:\bin\manager.exe"):
                install_autostart(8321)
        assert vbs.exists()
        content = vbs.read_text()
        # Launches jacked either way: pythonw -m manager (preferred) or the
        # manager.exe trampoline as fallback. Host-free in both.
...
class TestInstallAutostartFailure:
    ...
    def test_returns_error_when_jacked_not_on_path(self):
        from manager.service.platform import install_autostart
        with patch("manager.findbin.find_bin", return_value=None):
            result = install_autostart(8321)
    ...
    def test_plist_uses_resolved_binary_path(
        ...
        from manager.service.platform import install_autostart
        with patch("manager.service.platform._get_launchd_plist_path", return_value=plist):
            with patch("manager.findbin.find_bin", return_value="/Users/test/.local/bin/jacked"):
                install_autostart(8321)
        content = plist.read_text()
