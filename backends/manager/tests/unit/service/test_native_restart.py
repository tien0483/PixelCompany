"""Tests for manager.service.platform.native_restart() across macOS + Linux + Windows."""
...
class TestMacOS:
    def test_kickstart_called_when_plist_exists(self, tmp_path, monkeypatch):
        from manager.service import platform as plat
        monkeypatch.setattr(sys, "platform", "darwin")

        fake_plist = tmp_path / "ai.hank.manager.plist"
...
class TestLinux:
    def test_systemctl_called_when_user_unit_exists(self, tmp_path, monkeypatch):
        ...
        fake_unit = tmp_path / "manager.service"
...
class TestServiceRestartCli:
    ...
    @patch("manager.service.platform.native_restart", return_value=(True, "launchctl kickstart"))
    ...
    @patch("manager.service.platform.ensure_native_lifecycle",
           return_value=(True, "already_installed", "plist installed"))
    @patch("manager.service.platform.native_restart", return_value=(False, "no native manager"))
    @patch(
...
class TestUpdaterUsesNativeRestart:
    ...
    @patch("manager.install_method.detect_install_method", return_value="uv")
    @patch("manager.install_method.can_auto_upgrade", return_value=(True, ""))
    @patch("manager.service.platform.native_restart", return_value=(False, "no plist"))
    @patch("manager.service.updater.is_port_available")
    @patch("manager.service.updater.find_bin")
    @patch("subprocess.run")
    @patch("subprocess.Popen")
