...
def test_init_creates_file_with_metadata(tmp_path):
    from manager.service.update_status import init_status, read_status
...
def test_begin_phase_appends_entry(tmp_path):
    from manager.service.update_status import init_status, begin_phase, read_status
    p = tmp_path / "status.json"
...
def test_end_phase_ok(tmp_path):
    from manager.service.update_status import (
...
def test_end_phase_raises_on_unknown_phase(tmp_path):
    from manager.service.update_status import init_status, end_phase
    import pytest
    p = tmp_path / "status.json"
    init_status(p, from_version="a", to_version="b", method="uv")
...
def test_clear_status_missing_is_noop(tmp_path):
    from manager.service.update_status import clear_status
    clear_status(tmp_path / "nope.json")


def test_read_missing_returns_none(tmp_path):
    from manager.service.update_status import read_status
...
def test_read_with_mtime_returns_iso_timestamp(tmp_path):
    from manager.service.update_status import read_status_with_mtime, init_status
    p = tmp_path / "status.json"
    init_status(p, from_version="a", to_version="b", method="uv")
...
def test_write_is_atomic_no_tmp_leftover(tmp_path):
    from manager.service.update_status import init_status
    p = tmp_path / "status.json"
    init_status(p, from_version="a", to_version="b", method="uv")
...
def test_lock_rejects_second_init_if_another_active(tmp_path):
    from manager.service.update_status import init_status, LockBusy
    import pytest
    p = tmp_path / "status.json"
    init_status(p, from_version="a", to_version="b", method="uv")
...
def test_lock_allows_init_after_previous_succeeded(tmp_path):
    from manager.service.update_status import init_status, mark_succeeded
    p = tmp_path / "status.json"
    init_status(p, from_version="a", to_version="b", method="uv")
    mark_succeeded(p)
...
def test_init_or_adopt_fresh_file_initializes(tmp_path):
    from manager.service.update_status import init_or_adopt_status, read_status
    p = tmp_path / "status.json"
...
def test_api_endpoint_returns_null_when_no_status_file(tmp_path, monkeypatch):
    ...
    from manager.api.main import app as _app
...
def test_mark_failed_on_missing_file_is_noop(tmp_path):
    from manager.service.update_status import mark_failed, read_status
