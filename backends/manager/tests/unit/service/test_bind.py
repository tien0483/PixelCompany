"""Tests for manager.service.bind — bind resolution + socket construction.
...
from manager.service.bind import (
    BindPlan,
    create_sockets,
    detect_tailscale_ip,
    resolve_bind,
...
def test_scope_tailscale_found(mem_db):
    ...
    with mock.patch(
        "manager.service.bind.detect_tailscale_ip", return_value="100.64.12.7"
    ):
        plan = resolve_bind(None, 8321, mem_db)
...
def test_scope_tailscale_not_found_fallback(mem_db, caplog):
    mem_db.set_setting("remote_access_enabled", "true")
    mem_db.set_setting("remote_access_scope", "tailscale")
    with mock.patch("manager.service.bind.detect_tailscale_ip", return_value=None):
        with caplog.at_level(logging.WARNING, logger="manager.service.bind"):
            plan = resolve_bind(None, 8321, mem_db)
    assert plan.mode == "tailscale"
...
def test_missing_scope_defaults_to_tailscale(mem_db, caplog):
    ...
    mem_db.set_setting("remote_access_enabled", "true")
    with mock.patch(
        "manager.service.bind.detect_tailscale_ip", return_value="100.64.1.1"
    ):
        with caplog.at_level(logging.WARNING, logger="manager.service.bind"):
            plan = resolve_bind(None, 8321, mem_db)
    assert plan.mode == "tailscale"
...
def test_garbage_scope_defaults_to_tailscale_and_warns(mem_db, caplog):
    ...
    with mock.patch(
        "manager.service.bind.detect_tailscale_ip", return_value="100.64.2.2"
    ):
        with caplog.at_level(logging.WARNING, logger="manager.service.bind"):
            plan = resolve_bind(None, 8321, mem_db)
    assert plan.mode == "tailscale"
...
def test_default_db_constructed_lazily_when_none(mem_db):
    ...
    mem_db.set_setting("remote_access_enabled", "true")
    mem_db.set_setting("remote_access_scope", "all")
    with mock.patch("manager.web.database.Database", return_value=mem_db) as ctor:
...
def test_socket_set_tailscale_found_pairs_loopback_and_ip(mem_db):
    ...
    mem_db.set_setting("remote_access_scope", "tailscale")
    with mock.patch(
        "manager.service.bind.detect_tailscale_ip", return_value="100.64.99.9"
    ):
        plan = resolve_bind(None, 8321, mem_db)
...
def test_detect_udp_in_range_short_circuits_cli():
    ...
    with mock.patch("manager.service.bind.socket.socket", return_value=sock):
        with mock.patch("manager.service.bind.subprocess.run") as run:
...
def test_detect_udp_out_of_range_falls_to_cli():
    ...
    with mock.patch("manager.service.bind.socket.socket", return_value=sock):
        with mock.patch("manager.service.bind.subprocess.run", return_value=cli):
...
def test_detect_cli_file_not_found():
    ...
    with mock.patch("manager.service.bind.socket.socket", return_value=sock):
        with mock.patch(
            "manager.service.bind.subprocess.run", side_effect=FileNotFoundError
        ):
...
def test_detect_cli_timeout():
    ...
    with mock.patch("manager.service.bind.socket.socket", return_value=sock):
        with mock.patch("manager.service.bind.subprocess.run", side_effect=err):
            assert detect_tailscale_ip() is None
...
def test_create_sockets_conflict_names_address_and_closes_earlier(socket_cleanup):
    ...
    with mock.patch("manager.service.bind.socket.socket", side_effect=_spy):
...
def test_create_sockets_uses_reuseaddr_on_posix(socket_cleanup, monkeypatch):
    ...
    from manager.service import bind as bind_mod
