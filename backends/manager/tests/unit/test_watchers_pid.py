...
Now delegates to manager.service.process.is_process_alive and fail-safe
...
def test_returns_false_for_none_or_zero():
    from manager.api.watchers import _is_pid_alive
...
def test_fail_safe_on_unexpected_exception():
    ...
    from manager.api import watchers
    with patch(
        "manager.service.process.is_process_alive",
