"""Tests for manager.version_check module."""

import json
import time
from unittest.mock import patch, MagicMock

from manager import version_check as vc
...
class TestIsNewer:
    ...
    def test_newer_patch(self):
        """
        >>> from manager.version_check import is_newer
...
class TestCheckVersionCachedDualEndpoint:
    ...
    def _isolate_cache(tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("manager.version_check.VERSION_CACHE", cache)
        return cache
    ...
    def test_outdated_false_when_simple_lags_pypi(self, tmp_path, monkeypatch):
        ...
        with patch(
            "manager.version_check.get_latest_pypi_version", return_value="0.45.3"
        ), patch(
            "manager.version_check.get_latest_from_simple_index", return_value="0.45.2"
        ):
    ...
    def test_outdated_true_when_both_endpoints_agree(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            "manager.version_check.get_latest_from_simple_index", return_value="0.45.3"
        ):
            result = vc.check_version_cached("0.45.2", force=True)
        assert result["outdated"] is True
    ...
    def test_outdated_false_when_simple_unreachable(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            "manager.version_check.get_latest_from_simple_index", return_value=None
        ):
            result = vc.check_version_cached("0.45.2", force=True)
        assert result["outdated"] is False
    ...
    def test_outdated_false_when_pypi_unreachable(self, tmp_path, monkeypatch):
        self._isolate_cache(tmp_path, monkeypatch)
        with patch(
            "manager.version_check.get_latest_pypi_version", return_value=None
        ), patch(
            "manager.version_check.get_latest_from_simple_index", return_value="0.45.3"
        ):
            result = vc.check_version_cached("0.45.2", force=True)
    ...
    def test_outdated_false_when_simple_returns_garbage_version(
        ...
        with patch(
            ...
            "manager.version_check.get_latest_from_simple_index",
    ...
    def test_probe_failure_cache_expires_at_short_ttl(self, tmp_path, monkeypatch):
        ...
        with patch(
            "manager.version_check.get_latest_pypi_version", return_value="0.45.4"
        ) as p_pypi, patch(
            "manager.version_check.get_latest_from_simple_index", return_value="0.45.4"
        ) as p_simple:
