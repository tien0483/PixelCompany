...
class TestDetectInstallMethod:
    def test_detects_uv_from_path(self):
        ...
        with patch("manager.install_method.Path.resolve", return_value=fake):
    ...
    def test_defaults_to_pip_for_unknown_path(self, monkeypatch):
        ...
        with patch("manager.install_method.Path.resolve", return_value=fake):
            with patch("sys.executable", str(fake)):
                with patch("manager.install_method._is_path_under_any_site_packages", return_value=True):
...
class TestUpgradeCommand:
    def test_uv_method_uses_uv_tool_install(self):
        with patch("manager.install_method.detect_install_method", return_value="uv"):
    ...
    def test_pipx_method_uses_pipx_install_force(self):
        with patch("manager.install_method.detect_install_method", return_value="pipx"):
            cmd = upgrade_command(extras="tray")
        assert cmd[0] == "pipx"
    ...
    def test_pip_method_raises_valueerror(self):
        ...
        with patch("manager.install_method.detect_install_method", return_value="pip"):
    ...
    def test_editable_method_raises_valueerror(self):
        with patch("manager.install_method.detect_install_method", return_value="editable"):
            with pytest.raises(ValueError, match="pip auto-upgrade is not supported"):
                upgrade_command(extras="tray")
