"""Unit tests for manager.web.analytics_paths — cross-platform path resolution
...
from manager.web.analytics_paths import (
    decode_project_hash,
...
class TestDecodeProjectHash:
    def test_simple_project_with_fs(self, tmp_path):
        ...
        with patch(
            "manager.web.analytics_paths._probe_filesystem"
        ) as mock_probe:
...
class TestProbeFilesystem:
    def test_merges_dashed_dirs(self, tmp_path):
        ...
        from manager.web.analytics_paths import _probe_filesystem
