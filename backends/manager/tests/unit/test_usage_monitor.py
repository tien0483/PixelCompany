...
def _reset_monitor_state():
    ...
    explicitly.
    """
    import manager.api.usage_monitor as mod
...
class TestNoActionWhenDisabled:
    def test_active_poll_no_action_when_disabled(self):
        ...
        async def _run():
            with (
                patch(
                    "manager.api.usage_monitor.asyncio.sleep",
                    side_effect=_sleep_canceller(max_sleeps=1),
                ),
                patch(
                    "manager.web.auth.fetch_usage",
    ...
    def test_auto_swap_triggers_on_high_usage(self):
        ...
        async def _run():
            with (
                ...
                patch(
                    "manager.api.usage_monitor._read_active_account_id",
                    return_value=1,
                ),
                patch(
                    "manager.web.auth.fetch_usage",
                ...
                patch(
                    "manager.api.credential_helpers.read_fresh_active_token",
    ...
    def test_no_target_logs_warning(self, caplog):
        ...
        async def _run():
            with (
                ...
                with caplog.at_level(logging.WARNING, logger="manager.api.usage_monitor"):
                    with pytest.raises(asyncio.CancelledError):
                        await active_account_poll_loop(app)
    ...
    def test_window_keeper_pings_during_active_hours(self):
        ...
        async def _run():
            with (
                ...
                patch(
                    "manager.web.window_keeper.is_active_hours",
                    return_value=True,
                ),
                patch(
                    "manager.web.window_keeper.is_prewake_time",
                    return_value=False,
                ),
                patch(
                    "manager.web.window_keeper.needs_ping",
                    return_value=True,
                ),
                patch(
                    "manager.web.window_keeper.ping_account",
                    new_callable=AsyncMock,
                    return_value=True,
                ) as mock_ping,
            ):
    ...
    def test_ping_failure_schedules_backoff_and_next_sweep_skips(self, caplog):
        ...
        async def _run():
            with (
                ...
                patch(
                    "manager.web.auth.refresh_cc_token",
                    new_callable=AsyncMock,
                    return_value=False,
                ),
            ):
                with caplog.at_level(
                    logging.DEBUG, logger="manager.api.usage_monitor",
                ):
                    with pytest.raises(asyncio.CancelledError):
                        await full_sweep_loop(app)
    ...
    def test_burn_rate_skips_unchanged(self):
        ...
        async def _run():
            with (
                ...
                patch(
                    "manager.web.auto_swap.update_burn_rate",
                ) as mock_update_br,
            ):
                with pytest.raises(asyncio.CancelledError):
                    await active_account_poll_loop(app)
...
class TestEmergencePersistence:
    ...
    def test_first_emerge_suppressed(self):
        from manager.api.usage_monitor import _apply_emergence_persistence
...
class TestSilentStallWatchdog:
    ...
    def test_pattern_a_multi_account_stale(self):
        from manager.api.usage_monitor import _evaluate_stall
...
class TestSameTierDeficitAdvisory:
    ...
    def _applies(self, **overrides):
        from manager.api.usage_monitor import _same_tier_advisory_applies
        kwargs = dict(
            decision_action="stay",
            best={"id": 1},
            reason=None,
...
def _patch_swap_credential_helpers(monkeypatch, captured, lock_acquired=True):
    ...
    monkeypatch.setattr(
        "manager.api.credential_helpers.acquire_claude_lock", fake_lock,
    )
    monkeypatch.setattr(
        "manager.api.credential_helpers.sync_credential_to_all_stores",
        fake_sync,
    ...
    monkeypatch.setattr(
        "manager.api.credential_helpers.invalidate_live_cred_cache",
        lambda: None,
    )
    monkeypatch.setattr(
...
def _run_execute_swap(captured, reason="higher tier emerged: T0 vs T2",
    ...
    from manager.api import usage_monitor as um
...
class TestTriggerTaxonomy:
    ...
    def test_trigger_taxonomy_mapping(self):
        from manager.api.usage_monitor import _trigger_for_reason
...
class TestActivePollWatchdogLifespanWiring:
    ...
    def test_lifespan_imports_watchdog_loop(self):
        import inspect
        from manager.api import main as api_main
...
class TestCandidateStalenessOverride:
    def test_override_on_past_5h_reset_with_saturated_cache(self):
        from manager.api.usage_monitor import _candidate_staleness_override
        now = datetime.now(timezone.utc)
...
class TestPollIntervalClamp:
    def test_clamps_to_90_with_t0_deficit_candidate(self):
        from manager.api.usage_monitor import _clamp_poll_interval
        now = datetime.now(timezone.utc)
...
def test_all_accounts_loop_polls_codex_not_just_cc_accounts(monkeypatch):
    ...
    import manager.api.usage_monitor as um
    import manager.web.auth as auth_mod
