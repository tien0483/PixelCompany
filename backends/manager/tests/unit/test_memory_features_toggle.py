...
from manager.memory import vault as vault_mod
...
def test_enable_fresh_initializes_installs_entries_and_git_hooks(menv):
    ...
    from manager.memory.githook import HOOK_MARKER
...
def test_put_response_carries_engine_payload(monkeypatch):
    ...
    monkeypatch.setattr("manager.memory.setup.enable", fake_enable)
...
def test_put_memory_vault_foreign_origin_is_403(monkeypatch):
    monkeypatch.setattr("manager.memory.setup.enable", lambda home: {})
...
def test_init_already_initialized_reinstalls_hooks(menv):
    ...
    from manager.memory import settings_io
