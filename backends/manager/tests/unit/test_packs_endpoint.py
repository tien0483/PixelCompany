...
Every ``manager.packs`` boundary the routes touch is monkeypatched so the
...
(``packs.load_registry`` etc.), so patching ``manager.packs.*`` is seen at
call time.
...
import manager.packs as packs
from manager.api.routes.packs import router
