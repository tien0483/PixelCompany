...
from manager.codex import switching as sw
from manager.codex.switching import CodexSwapError
...
def test_swap_forces_file_storage(tmp_path, db):
    ...
    db.set_active_account_id(2, provider="codex")
    sw.swap_codex_account(db, 1, base=base)
    from manager.codex.credentials import credential_store_mode
...
def test_import_seeds_codex_slot(tmp_path, db, monkeypatch):
    from manager.codex.accounts import import_codex_account
