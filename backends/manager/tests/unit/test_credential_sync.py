...
def test_sync_writes_global_credential_file():
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=_WIN) as tmp:
        ...
        with (
            mock.patch("manager.api.credential_helpers.Path.home", return_value=tmp_path),
            mock.patch(
...
def test_read_platform_credentials_uses_user_account():
    ...
    with (
        mock.patch("manager.api.credential_helpers.sys") as mock_sys,
        mock.patch("manager.api.credential_helpers.subprocess.run", return_value=mock_result) as mock_run,
        mock.patch("manager.api.credential_helpers._get_keychain_username", return_value="testuser"),
    ):
...
def test_read_platform_credentials_linux():
    ...
    """
    with mock.patch("manager.api.credential_helpers.sys") as mock_sys:
...
def test_read_platform_credentials_keychain_not_found():
    ...
    with (
        mock.patch("manager.api.credential_helpers.sys") as mock_sys,
        mock.patch("manager.api.credential_helpers.subprocess.run", return_value=mock_result),
        mock.patch("manager.api.credential_helpers._get_keychain_username", return_value="testuser"),
    ):
        mock_sys.platform = "darwin"
        result = read_platform_credentials()
...
def test_write_platform_credentials_uses_hex_and_user():
    ...
    with (
        mock.patch("manager.api.credential_helpers.sys") as mock_sys,
        mock.patch(
            "manager.api.credential_helpers.subprocess.run",
...
def test_get_keychain_username_from_user():
    ...
    with mock.patch.dict(os.environ, {"USER": "alice"}, clear=False):
        from manager.api.credential_helpers import _get_keychain_username
...
def test_update_claude_config_email_creates_file():
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=_WIN) as tmp:
        ...
        with mock.patch(
            "manager.api.credential_helpers.Path.home", return_value=tmp_path
        ):
...
def test_read_fresh_active_token_from_file():
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=_WIN) as tmp:
        ...
        with (
            mock.patch("manager.api.credential_helpers.Path.home", return_value=tmp_path),
            mock.patch(
                "manager.api.credential_helpers.read_platform_credentials",
                return_value=None,
            ),
        ):
