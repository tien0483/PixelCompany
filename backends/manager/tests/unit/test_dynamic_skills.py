...
class TestGetValidSkillNames:
    def test_missing_skills_dir_returns_empty(self, tmp_path):
        with mock.patch("manager.api.routes.features.DATA_ROOT", tmp_path):
...
def skill_env(tmp_path):
    ...
    with mock.patch("manager.api.routes.features.DATA_ROOT", data_root), \
         mock.patch("manager.api.routes.features.CLAUDE_DIR", claude_dir), \
         mock.patch("manager.api.routes.features.SETTINGS_JSON", tmp_path / "settings.json"):
