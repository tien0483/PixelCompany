...
from manager import guardrails, statusline, statusline_setup
from manager.api.routes import features
from manager.api.routes.features import router
from manager.cli import main
from manager.memory.settings_io import SettingsUnreadableError, settings_path
...
def test_build_command_is_an_absolute_quoted_module_invocation():
    ...
    assert command.endswith('" -m manager.statusline')
...
@pytest.mark.parametrize("command", [
    ...
    "ccusage statusline",
    None,
    {"command": '"/usr/bin/python3" -m manager.statusline'},
    "",
    123,
])
...
def test_ensure_entry_overwrites_a_stale_jacked_command():
    settings = {"statusLine": {"type": "command", "command": '"/old/python" -m manager.statusline'}}
...
def test_sync_on_install_migrates_a_stale_jacked_command(home):
    settings = {
        "statusLine": {"type": "command", "command": '"/old/python" -m manager.statusline'}
    }
...
def _stub_uninstall_externals(env):
    ...
    import manager.codex.installer as cdx
    from manager import packs as packs_mod
...
def test_install_registers_the_statusline(clienv):
    ...
    assert "-m manager.statusline" in entry["command"]
...
def test_statusline_enable_disable_status_round_trip(clienv):
    ...
    assert "-m manager.statusline" in out
...
def _make_guarded_app() -> FastAPI:
    from manager.api.security import HostValidationMiddleware, build_allowed_origins
    app = FastAPI()
    app.include_router(router, prefix="/api")
...
def test_put_statusline_foreign_origin_is_403(monkeypatch):
    monkeypatch.setattr("manager.statusline_setup.enable", lambda home: {})
