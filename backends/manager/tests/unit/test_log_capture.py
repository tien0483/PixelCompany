"""Unit tests for manager.api.log_capture — ring buffer + handler."""

import asyncio
import logging
import threading

from manager.api.log_capture import MAX_FLUSH_ENTRIES, ServerLogBuffer, _format_entry
...
def test_websocket_logger_skips_broadcast_scheduling():
    """Events from manager.api.websocket logger only buffer, don't schedule.
    ...
    >>> buf = ServerLogBuffer(maxlen=10)
    >>> record = logging.LogRecord("manager.api.websocket", logging.DEBUG, "", 0, "prune", (), None)
...
def test_flush_async_logs_broadcast_failure(caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="manager.api.log_capture"):
