...
def test_use_account_success(client, tmp_path):
    ...
    with mock.patch(
        ...
        "manager.api.credential_helpers.reconcile_outgoing_credentials"
    ), mock.patch(
...
def test_use_account_arms_auto_swap_pause(client, db):
    ...
    with mock.patch(
        ...
        "manager.api.usage_monitor.note_external_swap"
    ) as mock_note:
        resp = client.post("/api/auth/accounts/1/use")
...
def test_use_account_rejected_does_not_arm_pause(client, db):
    ...
    with mock.patch("manager.api.usage_monitor.note_external_swap") as mock_note:
...
def test_refresh_usage_uses_fresh_token_for_active_account(client, tmp_path):
    ...
    with (
        ...
        mock.patch(
            "manager.api.routes.auth.fetch_usage",
...
def test_active_credential_refresh_token_match(client, db, tmp_path):
    ...
    with mock.patch(
        ...
        with mock.patch("manager.api.routes.auth.Path.home", return_value=tmp_path):
...
def test_active_credential_email_org_fallback(client, db, tmp_path):
    ...
    with (
        mock.patch("manager.api.routes.auth.Path.home", return_value=tmp_path),
        mock.patch(
            "manager.api.credential_helpers.read_platform_credentials",
