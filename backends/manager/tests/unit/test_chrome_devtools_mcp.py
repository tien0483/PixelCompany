...
class TestGetStatus:
    def test_not_installed_when_claude_missing(self):
        with mock.patch("manager.cli._run_claude_mcp", return_value=None):
    ...
    def test_not_configured_on_nonzero_exit(self):
        ...
        with mock.patch("manager.cli._run_claude_mcp", return_value=fake):
            status = _get_chrome_devtools_mcp_status()
            assert status["installed"] is False
...
class TestInstall:
    def test_skips_when_already_installed(self):
        ...
        with mock.patch("manager.cli._run_claude_mcp", return_value=get_result) as m:
    ...
    def test_installs_when_not_configured(self):
        ...
        with mock.patch("manager.cli._run_claude_mcp", side_effect=fake_run):
            _install_chrome_devtools_mcp()
...
class TestRemove:
    def test_returns_true_on_success(self):
        ok = subprocess.CompletedProcess(args=[], returncode=0, stdout="", stderr="")
        with mock.patch("manager.cli._run_claude_mcp", return_value=ok):
            assert _remove_chrome_devtools_mcp() is True

    def test_returns_false_on_failure(self):
        fail = subprocess.CompletedProcess(args=[], returncode=1, stdout="", stderr="")
        with mock.patch("manager.cli._run_claude_mcp", return_value=fail):
...
class TestSetMode:
    ...
    def test_sets_autoconnect_mode(self):
        ...
        with mock.patch("manager.cli._run_claude_mcp", return_value=ok_result):
