...
from manager.api.credential_helpers import build_oauth_data
from manager.web.auth import refresh_cc_token, should_refresh_cc
...
class TestResolveAccountCCMatch:
    @patch("manager.launch.find_bin", return_value="/usr/bin/claude")
    @patch("manager.launch.read_platform_credentials")
    @patch("manager.launch.Path.home")
...
class TestAccountResponseCCFields:
    def test_has_cc_token_true_when_present(self, tmp_path):
        ...
        from manager.api.routes.auth import _account_to_response
...
class TestRefreshCCTokenSuccess:
    ...
    @patch("manager.web.auth.httpx.AsyncClient")
...
class TestCCInvalidGrantHandling:
    ...
    @patch("manager.web.auth.httpx.AsyncClient")
    @patch("manager.api.credential_helpers.read_platform_credentials", return_value=None)
...
class TestRefreshCCTokenLockHeld:
    ...
    def test_returns_true_without_http_call_when_locked(self, mock_client_cls, tmp_path):
        ...
        async def _run():
            from manager.web.auth import _get_cc_refresh_lock
...
class TestCcFlowDedup:
    ...
    def test_dedup_returns_existing_flow_id(self, tmp_path):
        ...
        from manager.web.oauth import _active_flows
