...
class TestProfilePiggyback:
    def test_successful_fetch_triggers_profile_refresh(self):
        _reset_state()
        db = _mock_db()
        with patch("manager.web.auth.httpx.AsyncClient", return_value=_mock_client()), \
             patch("manager.web.auth.fetch_profile", new=AsyncMock()) as prof:
    ...
    def test_refresh_throttled_within_interval(self):
        _reset_state()
        db = _mock_db()
        with patch("manager.web.auth.httpx.AsyncClient", side_effect=lambda **k: _mock_client()), \
             patch("manager.web.auth.fetch_profile", new=AsyncMock()) as prof, \
             patch("manager.web.auth._USAGE_MANUAL_FLOOR_SECONDS", 0), \
             patch("manager.web.auth._USAGE_FETCH_MIN_INTERVAL", 0, create=True):
    ...
    def test_profile_failure_is_nonfatal_and_does_not_retry_every_poll(self):
        ...
        with patch("manager.web.auth.httpx.AsyncClient", side_effect=lambda **k: _mock_client()), \
             patch("manager.web.auth.fetch_profile", new=boom):
            result = asyncio.run(fetch_usage(1, db, manual=True))
