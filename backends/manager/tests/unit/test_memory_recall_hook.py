...
from manager.hooks import memory_recall
from manager.memory import recall
...
def test_shim_swallows_downstream_exception(env, capsys):
    ...
    with patch("manager.memory.recall.build_brief", side_effect=_boom):
...
def test_decision_poor_group_never_full_scans(env):
    ...
    env.monkeypatch.setattr("manager.memory.recall.vault_mod.read_note", counting_read)
    brief = recall.build_brief(str(_cwd_dir(env, "myrepo")))
    assert brief
