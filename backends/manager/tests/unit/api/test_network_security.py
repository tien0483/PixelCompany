"""Tests for manager.api.security, the network-exposure hardening layer.
...
def client(monkeypatch):
    ...
    monkeypatch.setattr(
        "manager.version_check.check_version_cached",
...
def _run_middleware_http(headers: list[tuple[bytes, bytes]], method: str = "GET"):
    ...
    from manager.api.security import HostValidationMiddleware
...
def test_cors_middleware_honors_in_place_refresh(client):
    ...
    from manager.api.main import _cors_origins
