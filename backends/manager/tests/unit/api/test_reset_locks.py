...
def test_reset_locks_rebinds_bulk_refresh_lock() -> None:
    ...
    from manager.api.routes import auth as auth_routes
...
_REBIND_CASES: tuple[tuple[str, str], ...] = (
    ("manager.api.routes.features", "_settings_lock"),
    ("manager.api.routes.permissions", "_project_settings_lock"),
    ("manager.api.routes.system", "_upgrade_lock"),
)
...
def test_reset_locks_clears_per_account_refresh_locks() -> None:
    ...
    from manager.web import auth as web_auth
...
def test_permissions_uses_attribute_access_not_aliased_import() -> None:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module == "manager.api.routes.features":
            ...
            assert "_settings_lock" not in imported, (
                ...
                "`from manager.api.routes import features as _features_mod` + "
...
def test_permissions_sees_rebound_features_settings_lock() -> None:
    ...
    """
    from manager.api.routes import features, permissions  # noqa: F401
