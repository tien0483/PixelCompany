---
description: "Parallel UX checks — standalone (generated 2026-07-21; upgrade jacked + re-run /jacked-setup to update)"
---
...
- **Dev Server Port**: 8321 (the jacked dashboard; user-managed in a separate terminal — do NOT auto-start or kill it)
- **Component Paths**: `jacked/data/web/js/components/`, plus page shells + CSS under `jacked/data/web/`

## Credential Hints
none found — the local dashboard has no login. Only ask the user about creds if a gated view is added.

## UX Focus Areas
- **Multi-account dashboard** (the core surface): exercise the account list/dropdown at 1 / few / many accounts; verify the active-account pill, the switch flow, and per-account usage cards stay correct and update live.
- **Cross-view state**: settings/feature toggles, the actions (⋯) menu, and usage refresh all touch shared state — confirm one change doesn't leave another view stale.
- **Memory Vault surfaces**: vault status, settings toggles, and the unreadable-settings banner — verify enable/disable round-trips and error states read clearly.
- **Dark theme contrast** + responsive widths (≈1366 / 820 / 375); empty / loading / error states for every panel.
- **Discoverability**: common actions (add account, switch, refresh, open settings) should be reachable without hunting.
...
3. If EITHER differs, count the intervening commits (`git rev-list --count <generated-commit>..HEAD 2>/dev/null`) and surface one line, then continue:
   > ⚠️ This `/ux` config was generated N commits ago and the stack/paths may have drifted — re-run `/jacked-setup ux` to refresh.
