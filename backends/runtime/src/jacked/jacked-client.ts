// Read-mostly client for the claude-jacked dashboard API on 127.0.0.1:8321.
//
// claude-jacked is an optional companion process. Every call here resolves to null or a
...
/**
 * Providers claude-jacked can hold accounts for, and what Kanban may do with them.
 *
 ...
 */
const PROVIDER_CAPABILITIES: Record<RuntimeJackedProvider, { canAutoSwap: boolean; canTrackUsage: boolean }> = {
...
const FEATURE_CATEGORIES: RuntimeJackedFeatureCategory[] = ["agents", "commands", "hooks", "knowledge"];

export interface JackedClient {
	/** Resolved companion base URL (no trailing slash). */
	baseUrl: string;
	fetchSnapshot: () => Promise<RuntimeJackedSnapshot | null>;
	setFeatureEnabled: (
		category: RuntimeJackedFeatureCategory,
		name: string,
		enabled: boolean,
	...
	fetchPacks: () => Promise<RuntimeJackedPacks | null>;
	...
	fetchUsageOverview: (days?: number) => Promise<RuntimeJackedUsageOverview | null>;
	fetchSwapLog: (limit?: number) => Promise<RuntimeJackedSwapLog | null>;
	/** Start Claude OAuth only (POST /api/auth/accounts/add?provider=claude). */
	startClaudeOAuth: (remote?: boolean) => Promise<RuntimeJackedOAuthStartResponse>;
...
export interface CreateJackedClientDependencies {
	baseUrl?: string;
...
function parseProvider(value: unknown): RuntimeJackedProvider {
...
function parseFeatures(raw: unknown): RuntimeJackedFeature[] {
	if (!isRecord(raw)) {
		return [];
	}
	const features: RuntimeJackedFeature[] = [];
...
function parseSwapEntry(raw: unknown): RuntimeJackedSwap | null {
	if (!isRecord(raw)) {
		return null;
	}
...
function parseLatestSwap(raw: unknown): RuntimeJackedSwap | null {
	if (!isRecord(raw)) {
		return null;
	}
...
function parseInstallationsOverview(raw: unknown): RuntimeJackedInstallationsOverview | null {
	if (!isRecord(raw)) {
		return null;
	}
	...
	const projects: RuntimeJackedInstallationsOverview["projects"] = [];
...
export function createJackedClient(deps: CreateJackedClientDependencies): JackedClient {
	...
	const request = async (
		...
		try {
			...
			if (!didWarnUnreachable) {
				...
				deps.warn(`claude-jacked is not reachable at ${baseUrl}; office vitality data is disabled.`);
			}
			return null;
		} finally {
	...
	const fetchSnapshot = async (): Promise<RuntimeJackedSnapshot | null> => {
	 ...
	 */
	const startOAuthFlow = async (path: string): Promise<RuntimeJackedOAuthStartResponse> => {
		const controller = new AbortController();
		const timeout = setTimeout(() => {
			controller.abort();
	...
	return {
		...
		fetchActiveSessions: async () => {
			...
			const sessions: RuntimeJackedSession[] = [];
		...
		fetchServerLogs: async (limit = 100) => {
			...
			const entries: RuntimeJackedServerLogs["entries"] = [];
		...
		fetchHookLogs: async (limit = 50) => {
			...
			const logs: RuntimeJackedHookLogs["logs"] = [];
		...
		fetchUsageOverview: async (days = 1) => {
			...
			if (raw === null) {
				return {
					...
					flagCount: 0,
					ready: false,
					error: "claude-jacked is not reachable.",
				};
			}
			if (!isRecord(raw)) {
				return null;
		...
		fetchSwapLog: async (limit = 10) => {
			...
			const swaps: RuntimeJackedSwapLog["swaps"] = [];
		...
		submitOAuthCode: async (flowId, code) => {
			...
			try {
				...
				return {
					...
					authUrl: null,
					mode: null,
					submitError: "claude-jacked is not reachable.",
				};
			} finally {
				clearTimeout(timeout);
			}
		...
		proxyRequest: async (method, jackedPath, body = null, contentType = null) => {
			...
			try {
				...
				return {
					status: 502,
					body: JSON.stringify({ error: "claude-jacked is not reachable." }),
