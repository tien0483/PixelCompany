...
function renderLinearSetupGuidanceForAgent(agentId: RuntimeAgentId | null): string {
	switch (agentId) {
		...
		case "orchestrator":
			return "- If delegation tools are missing, check runtime.orchestrator.status in the Agents sidebar — dsh plugins auto-install into $DSH_HOME/profiles/headless; pick a deployed flow under Custom agent (flow) on the card to mount it as an mcp__* tool.";
		case "opencode":
...
- If your current working directory is inside \`${KANBAN_TASK_WORKTREES_HOME_DIR_NAME}/\` (or the legacy \`${LEGACY_KANBAN_TASK_WORKTREES_HOME_DIR_NAME}/\`), you are inside a Kanban task worktree. In that case, create or manage tasks against the main workspace path, not the task worktree path. Pass the main workspace with \`--project-path\`.
