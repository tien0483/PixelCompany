...
/** Worktrees created before the home-directory rename; still live, never moved. */
export function getLegacyTaskWorktreesHomePath(): string {
	return join(homedir(), LEGACY_RUNTIME_HOME_PARENT_DIR_NAME, RUNTIME_WORKTREES_DIR);
}

/** Runtime home written before the rename; the migration source. */
export function getLegacyRuntimeHomePath(): string {
	return join(homedir(), LEGACY_RUNTIME_HOME_PARENT_DIR_NAME, RUNTIME_HOME_DIR);
}
