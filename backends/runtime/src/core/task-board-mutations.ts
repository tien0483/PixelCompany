 ...
 * conventional review-before-done flow). Chain followers unlock regardless of which column
 * they wait in — a chain root can be dragged straight from In Progress to Done (skipping
 * Review) and its follower must still start, since the shared worktree is otherwise left
 * stalled forever.
 */
export function getReadyLinkedTaskIdsAfterLeavingReview(
	...
	for (const dependency of board.dependencies) {
		...
		// A chain follower gets the same skip-review parity it has in In Progress below —
		// otherwise a root dragged straight to Done strands the rest of its chain.
		if (waiterColumnId === "backlog") {
			if (fromColumnId === "review" || dependency.chain === true) {
