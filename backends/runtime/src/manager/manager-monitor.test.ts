...
describe("resolveManagerMonitorState", () => {
	...
	it("stays null when there was never a successful fetch", () => {
		expect(resolveManagerMonitorState(null, null)).toBeNull();
	});
	...
	it("clears stale on a successful fetch", () => {
		...
		const next = resolveManagerMonitorState(prior, fresh);
...
describe("createManagerMonitor LKG", () => {
	it("does not wipe cached state when refresh fails after success", async () => {
		...
		const monitor = createManagerMonitor({
