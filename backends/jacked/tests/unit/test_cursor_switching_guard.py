...
from jacked.cursor.switching import CursorSwapError, is_cursor_running, swap_cursor_account


def test_is_cursor_running_fail_closed_on_probe_error():
    with patch("jacked.cursor.switching.subprocess.run", side_effect=OSError("boom")):
        assert is_cursor_running() is True


def test_is_cursor_running_windows_matches_helper():
    with patch("jacked.cursor.switching.os.name", "nt"):
        with patch(
            "jacked.cursor.switching.subprocess.run",
...
def test_swap_refuses_when_registry_allows_auto_swap(monkeypatch):
    monkeypatch.setattr("jacked.cursor.switching.can_auto_swap", lambda _p: True)
