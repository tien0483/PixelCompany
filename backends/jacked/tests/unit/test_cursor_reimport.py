...
from jacked.cursor.accounts import (
    CursorImportError,
    CursorReimportError,
    cursor_account_slot,
...
from jacked.cursor.credentials import CursorCredentialStatus, CursorIdentity
...
def test_import_persists_cursor_row(db, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "jacked.cursor.accounts.detect_cursor_account",
        lambda *a, **k: _cursor_status(),
    )
    monkeypatch.setattr(
        "jacked.cursor.accounts.read_cursor_auth",
...
def test_ensure_cursor_launch_credential_reimports_when_slot_empty(db, tmp_path, monkeypatch):
    from jacked.cursor.accounts import ensure_cursor_launch_credential
