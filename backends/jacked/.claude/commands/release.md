---
description: "Release manager — standalone (generated 2026-07-21; upgrade jacked + re-run /jacked-setup to update)"
---
...
- **Base Branch**: master
- **Shipping Model**: pypi
- **Version Source**: `jacked/__init__.py` `__version__` (hatch dynamic version — pyproject.toml declares `dynamic = ["version"]` with `[tool.hatch.version] path = "jacked/__init__.py"`; do NOT add a static version to pyproject.toml)
- **Gate Commands**: `uv run python -m pytest` (mirrors `.github/workflows/tests.yml` exactly); `uv run ruff check .` (pinned ruff from the dev group — the pre-push lint gate)
- **Publish Mechanism**: GitHub Release published on `master` → `.github/workflows/publish.yml` → `python -m build` + CycloneDX SBOM → PyPI Trusted Publisher (OIDC, `pypa/gh-action-pypi-publish`) → https://pypi.org/p/claude-jacked. Cutting the GitHub Release (e.g. `gh release create vX.Y.Z`) IS the publish trigger — treat it as the irreversible step.
- **Verify**: poll `https://pypi.org/pypi/claude-jacked/json` until the new version appears; confirm the publish.yml run succeeded (`gh run list --workflow=publish.yml`); optionally `uv tool install --upgrade claude-jacked` and check `jacked --version`. Also confirm README's "Version History" table carries the new version's row before tagging.
...
3. If EITHER differs, count the intervening commits (`git rev-list --count <generated-commit>..HEAD 2>/dev/null`) and surface one line, then continue:
   > ⚠️ This `/release` config was generated N commits ago and the stack/paths may have drifted — re-run `/jacked-setup release` to refresh.
