...
class AccountResponse(BaseModel):
    ...
    # Provider capability flags (from jacked.providers) — UI greys out unsafe actions
...
def _account_to_response(row: dict, db=None) -> AccountResponse:
    ...
    from jacked.providers import capabilities_for
...
async def start_add_account(
    ...
    if provider == "antigravity":
        from jacked.antigravity.accounts import (
    ...
    if provider == "cursor":
        from jacked.cursor.accounts import CursorImportError, add_cursor_account
        from jacked.providers import capabilities_for
    ...
    if provider not in ("claude",):
        from jacked.providers import is_known_provider
...
async def list_providers():
    ...
    from jacked.providers import describe_providers
...
async def prepare_account_launch_dir(account_id: int, request: Request):
    ...
    Delegates to ``jacked.launch.prepare_account_dir`` — the same code path as
...
async def use_account(account_id: int, request: Request):
    ...
    if account.get("provider") == "antigravity":
        ...
        from jacked.antigravity.switching import (
    ...
    if account.get("provider") == "cursor":
        ...
        from jacked.cursor.switching import CursorSwapError, swap_cursor_account
        from jacked.providers import capabilities_for
