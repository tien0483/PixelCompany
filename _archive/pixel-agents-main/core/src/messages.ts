...
export interface SpriteGenProgress {
...
export interface SaveCustomAsset {
  type: 'saveCustomAsset';
  id?: string;
  name: string;
  category: AnonymousSchema_206;
  sprite: string[][];
  width: number;
  height: number;
  footprintW: number;
  ...
  canPlaceOnWalls?: boolean;
  canPlaceOnSurfaces?: boolean;
  orientations?: AnonymousSchema_218[];
}

export type AnonymousSchema_206 =
  | 'desks'
  | 'chairs'
  | 'storage'
  | 'electronics'
...
export interface AnonymousSchema_218 {
  orientation: AnonymousSchema_219;
  sprite: string[][];
  width: number;
  height: number;
...
export type AnonymousSchema_219 = 'front' | 'side' | 'back';
...
export interface GenerateSprite {
  type: 'generateSprite';
  requestId: string;
  prompt: string;
  seedSprite?: string[][];
