/**
 * RETIRED — Pixel Agents ingestion is disabled in the Kanban-integrated product.
 * See server/src/INGESTION_RETIRED.ts. Kanban owns agent activity via hooks.ingest.
 *
...
export function startStaleExternalAgentCheck(
  ...
  return setInterval(() => {
    ...
    for (const [id, agent] of agents) {
      ...
      // Despawn when the JSONL file is gone, OR when it's been idle (no new
      // writes) for longer than EXTERNAL_IDLE_DESPAWN_MS — so a quit/closed
      // session clears from the office instead of lingering. Re-adopts if the
      // session resumes and writes again. Runs regardless of hooks (SessionEnd
      // covers the graceful case; this is the backstop for quit/kill).
      try {
        const { mtimeMs } = fs.statSync(agent.jsonlFile);
        if (Date.now() - mtimeMs > EXTERNAL_IDLE_DESPAWN_MS) {
          toRemove.push(id);
        }
      } catch {
    ...
    for (const id of toRemove) {
      ...
      console.log(`[Pixel Agents] Watcher: Agent ${id} - removing stale/idle external agent`);
      agentRemovalCallback?.(id);
    }
  }, EXTERNAL_STALE_CHECK_INTERVAL_MS);
}
