...
import { writeCustomAsset } from './customAssetWriter.js';
import { readLayoutFromFile, writeLayoutToFile } from './layoutPersistence.js';
import { claudeProvider } from './providers/index.js';
import { handleGenerateSprite } from './spriteGen.js';
...
export function handleClientMessage(
  ...
  switch (msg.type) {
    ...
    case 'sendAgentRequest':
      // Standalone agents do not have a PTY terminal, so we cannot inject
      // text into their prompt. This action is VS Code-only.
      break;
    ...
    case 'saveCustomAsset':
      writeCustomAsset(msg as any);
      void ctx.onReloadAssets?.(send);
      break;

    case 'generateSprite':
      void handleGenerateSprite(msg as any, (prog) => send(prog as any));
      break;
