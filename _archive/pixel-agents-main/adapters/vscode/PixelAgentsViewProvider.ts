...
import { writeCustomAsset } from '../../server/src/customAssetWriter.js';
...
import { handleGenerateSprite } from '../../server/src/spriteGen.js';
import {
  getProjectDirPath,
  launchNewTerminal,
  restoreAgents,
...
export class PixelAgentsViewProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(webviewView: vscode.WebviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      ...
      } else if (message.type === 'sendAgentRequest') {
        const agent = this.store.get(message.id);
        if (agent) {
          if (agent.terminalRef) {
            agent.terminalRef.show();
            agent.terminalRef.sendText(message.text, true);
          } else if (agent.leadAgentId !== undefined) {
            const lead = this.store.get(agent.leadAgentId);
            if (lead?.terminalRef) {
              lead.terminalRef.show();
              lead.terminalRef.sendText(message.text, true);
            }
          }
        }
      ...
      } else if (message.type === 'saveCustomAsset') {
        writeCustomAsset(message as any);
        await this.reloadAndSendFurniture();
      } else if (message.type === 'generateSprite') {
        void handleGenerateSprite(message as any, (prog) => this.sendOrBuffer(prog as any));
