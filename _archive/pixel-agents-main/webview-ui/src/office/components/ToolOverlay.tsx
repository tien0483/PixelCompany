      ...
      {allIds.map((id) => {
        ...
        return (
          ...
          >
            ...
            {isSelected && ch.waitingAwaitingInput && !ch.isExternal && !isBrowserRuntime && (
              <div
                className="pixel-panel mt-2 p-2 flex items-center gap-2 pointer-events-auto"
                onClick={(e) => e.stopPropagation()}
              >
                <input
                  type="text"
                  className="bg-bg text-text border-2 border-border px-4 py-2 text-sm outline-none focus:border-accent w-64"
                  placeholder="Tell agent what to do..."
                  value={inputText}
                  onChange={(e) => setInputText(e.target.value)}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && inputText.trim()) {
                      transport.send({ type: 'sendAgentRequest', id, text: inputText.trim() });
                      setInputText('');
                    }
                  }}
                  autoFocus
