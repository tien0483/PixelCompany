...
interface EditorToolbarProps {
  ...
  // NPC handlers
  brushNpcPalette: number;
  brushNpcHue: number;
  brushNpcScript: import('../types.js').NpcAction[];
  onBrushNpcPaletteChange: (palette: number) => void;
  onBrushNpcHueChange: (hue: number) => void;
  onBrushNpcScriptChange: (script: import('../types.js').NpcAction[]) => void;
...
export function EditorToolbar({
  ...
  brushNpcHue,
  brushNpcScript,
  onBrushNpcPaletteChange,
  onBrushNpcHueChange,
  onBrushNpcScriptChange,
  onOpenPixelEditor,
}: EditorToolbarProps) {
  ...
  const isNpcsActive = activeTool === EditTool.NPCS;
  ...
  return (
    <div className="absolute bottom-76 left-10 z-10 pixel-panel p-4 flex flex-col-reverse gap-4 max-w-[calc(100vw-20px)]">
      ...
      <div className="flex gap-4 flex-wrap">
        ...
        <Button
          variant={isNpcsActive ? 'active' : 'default'}
          size="md"
          onClick={() => onToolChange(EditTool.NPCS)}
          title="Place NPCs"
        >
          NPCs
        </Button>
      ...
      {isNpcsActive && (
        <div className="flex flex-col-reverse gap-4 pb-2">
          <div className="flex gap-4 items-start bg-bg-dark border-2 border-border p-4 w-[500px]">
            <div className="flex flex-col gap-2">
              <label className="text-xs text-text-muted">Palette (0-3)</label>
              <input
                type="number"
                min="0" max="3"
                value={brushNpcPalette}
                onChange={(e) => onBrushNpcPaletteChange(parseInt(e.target.value) || 0)}
                className="bg-bg text-text p-1 border-2 border-border text-xs w-16"
              />
              <label className="text-xs text-text-muted">Hue (0-360)</label>
              <input
                type="number"
                min="0" max="360"
                value={brushNpcHue}
                onChange={(e) => onBrushNpcHueChange(parseInt(e.target.value) || 0)}
                className="bg-bg text-text p-1 border-2 border-border text-xs w-16"
              />
              <div className="text-xs text-text-muted mt-2 max-w-32">
                Click map to place.<br/>Click NPC to erase.
              </div>
            </div>
            <div className="flex flex-col gap-2 flex-1">
              <div className="text-xs font-bold text-text-muted mb-1">Action Script</div>
              <div className="flex flex-col gap-1 max-h-40 overflow-y-auto pixel-scrollbar pr-2">
                {brushNpcScript.map((action, i) => (
                  <div key={i} className="flex gap-2 items-center text-xs text-text bg-bg p-1 border border-border/50">
                    <span className="font-bold">{action.kind}</span>
                    {action.kind === 'walkTo' && (
                      <span className="flex-1">c:{action.col},r:{action.row}</span>
                    )}
                    {(action.kind === 'wander' || action.kind === 'typeAtSeat') && (
                      <span className="flex-1">{action.seconds}s</span>
                    )}
                    {action.kind === 'patrol' && (
                      <span className="flex-1">{action.waypoints.length} pts</span>
                    )}
                    <Button variant="ghost" size="sm" onClick={() => {
                      const next = [...brushNpcScript];
                      next.splice(i, 1);
                      onBrushNpcScriptChange(next);
                    }}>x</Button>
                  </div>
                ))}
              </div>
              <div className="flex gap-2 mt-2 flex-wrap">
                <Button size="sm" onClick={() => onBrushNpcScriptChange([...brushNpcScript, { kind: 'wander', seconds: 5 }])}>+ Wander</Button>
                <Button size="sm" onClick={() => onBrushNpcScriptChange([...brushNpcScript, { kind: 'typeAtSeat', seconds: 5 }])}>+ Type</Button>
                <Button size="sm" onClick={() => onBrushNpcScriptChange([...brushNpcScript, { kind: 'walkTo', col: 1, row: 1 }])}>+ WalkTo</Button>
                <Button size="sm" onClick={() => onBrushNpcScriptChange([...brushNpcScript, { kind: 'patrol', waypoints: [{col: 1, row: 1}, {col: 3, row: 3}] }])}>+ Patrol</Button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex gap-4 flex-wrap items-center">
            ...
            {activeCategory !== CARPET_CATEGORY_ID && (
              <Button
                ...
                title="Draw a custom pixel art asset"
              >
                Draw
              </Button>
            )}
