...
// ── Agent tether (sub-agent ↔ parent / teammate ↔ lead link line) ─
/** Dashed line color linking a sub-agent (or teammate) to its parent (or lead). */
export const AGENT_TETHER_COLOR = 'rgba(120, 170, 255, 0.55)';
/** Dash pattern (canvas setLineDash) for the tether. */
export const AGENT_TETHER_DASH: [number, number] = [4, 4];
/** Tether stroke width in device pixels. */
export const AGENT_TETHER_LINE_WIDTH = 1.5;
/** Dash-offset animation speed (dash-units per second) for a "message flowing" feel. */
export const AGENT_TETHER_DASH_SPEED = 12;
...
export const SPEECH_BUBBLE_DURATION_SEC = 4;
/** Max characters of a delegated-task label before it is truncated in a speech bubble / feed. */
export const DELEGATE_LABEL_MAX_CHARS = 40;
...
// ── Activity Feed (right-side subtitle of office events) ─────
/** Maximum number of lines retained in OfficeState.activityFeed (oldest dropped). */
export const ACTIVITY_FEED_MAX = 30;
/** Poll interval (ms) for the ActivityFeed component to mirror OfficeState.activityFeed. */
export const ACTIVITY_FEED_POLL_MS = 250;

// ── Scenario Bar (run scenario + record roleplay, standalone only) ─
/** HTTP endpoint returning the list of saved scenarios as [{ name, kind }]. */
export const API_SCENARIOS = '/api/scenarios';
/** HTTP endpoint to begin recording a roleplay to a named scenario. */
export const API_RECORD_START = '/api/record/start';
/** HTTP endpoint to stop the active recording (flushes + narrates). */
export const API_RECORD_STOP = '/api/record/stop';
/** HTTP endpoint returning the current recording status ({ recording, name }). */
export const API_RECORD_STATUS = '/api/record/status';
/** Poll interval (ms) for the REC indicator to mirror /api/record/status. */
export const RECORD_STATUS_POLL_MS = 2000;
/** URL query param that plays a recording scenario on load (see transport/index.ts). */
export const SCENARIO_REPLAY_PARAM = 'replay';
/** URL query param that plays a roleplay scenario on load (see transport/index.ts). */
export const SCENARIO_ROLEPLAY_PARAM = 'roleplay';

// ── Script Editor (author + save a roleplay scenario, standalone only) ─
/** schemaVersion stamped onto scenarios authored by the in-UI script editor. */
export const SCRIPT_EDITOR_SCHEMA_VERSION = 1;
/**
 * Selectable roleplay actions in the script-editor timeline. Mirrors the
 * `Scenario.script[].action` union in core/src/scenario.ts.
 */
...
/** Valid scenario name (filename) pattern — mirrors the server's save-route guard. */
export const SCENARIO_NAME_PATTERN = /^[a-zA-Z0-9_-]+$/;
/** Default time increment (ms) applied when appending a new script row. */
export const SCRIPT_ROW_TIME_STEP_MS = 1000;

/** Seek step (ms) for the replay controls' skip-back / skip-forward buttons. */
export const REPLAY_SKIP_MS = 5000;

/** Endpoint + poll interval for custom live-agent display names (name-file convention). */
export const API_AGENT_NAMES = '/api/agent-names';
export const AGENT_NAMES_POLL_MS = 3000;
