...
import { SpriteGenTab } from './components/spritegen/SpriteGenTab.js';
...
import { ActivityFeed } from './office/components/ActivityFeed.js';
import { AgentNamesSync } from './office/components/AgentNamesSync.js';
import { OfficeCanvas } from './office/components/OfficeCanvas.js';
import { ReplayControls } from './office/components/ReplayControls.js';
import { ScenarioBar } from './office/components/ScenarioBar.js';
import { SpeechOverlay } from './office/components/SpeechOverlay.js';
...
import { ReplayTransport } from './transport/replayTransport.js';
...
function getOfficeState(): OfficeState {
  if (!officeStateRef.current) {
    ...
    if (transport instanceof ReplayTransport) {
      officeStateRef.current.isReplayMode = true;
      transport.setOfficeState(officeStateRef.current);
    }
  }
...
function App() {
  ...
  const handleToggleControlMode = useCallback(() => {
    setIsControlMode((prev) => {
      const next = !prev;
      if (!next) {
        // When turning off control mode, clear all pinned tiles
        const state = getOfficeState();
        for (const ch of state.getCharacters()) {
          ch.pinnedTile = undefined;
        }
      }
      return next;
    });
  ...
  useEffect(() => {
    ...
    hooks.editorTileAction = editor.handleEditorTileAction;
    hooks.editorEraseAction = editor.handleEditorEraseAction;
    ...
    <div ref={containerRef} className="w-full h-full relative overflow-hidden">
      ...
      {!isDebugMode ? (
        <>
            ...
            (() => {
              ...
              return (
                <EditorToolbar
                  ...
                  brushNpcPalette={editor.brushNpcPalette}
                  brushNpcHue={editor.brushNpcHue}
                  brushNpcScript={editor.brushNpcScript}
                  onBrushNpcPaletteChange={editor.handleBrushNpcPaletteChange}
                  onBrushNpcHueChange={editor.handleBrushNpcHueChange}
                  onBrushNpcScriptChange={editor.handleBrushNpcScriptChange}
                />
              );
            })()}
          ...
          />
          <ActivityFeed officeState={officeState} />
          {isBrowserRuntime && <AgentNamesSync officeState={officeState} />}
          {/* Standalone-only: run a saved scenario / record a roleplay over plain
              HTTP. Hidden while replaying (ReplayControls owns that surface). */}
          {isBrowserRuntime && !(transport instanceof ReplayTransport) && (
            <ScenarioBar onOpenPainter={() => setIsPixelEditorOpen(true)} />
          )}
        </>
      ) : (
        <DebugView
      ...
      <SpriteGenTab
        ...
        onSave={(asset) => transport.send(asset as any)}
      />
      {transport instanceof ReplayTransport && (
        <>
          <NarrationOverlay transport={transport} />
          <ReplayControls transport={transport} getOfficeState={getOfficeState} />
        </>
      )}
    </div>
  );
