...
interface EditorActions {
  ...
  brushNpcHue: number;
  brushNpcScript: import('../office/types.js').NpcAction[];
  handleBrushNpcPaletteChange: (palette: number) => void;
  handleBrushNpcHueChange: (hue: number) => void;
  handleBrushNpcScriptChange: (script: import('../office/types.js').NpcAction[]) => void;
  handleNpcRemove: (id: number) => void;
}
...
export function useEditorActions(
  ...
  const [brushNpcPalette, setBrushNpcPaletteState] = useState(editorState.brushNpcPalette);
  const [brushNpcHue, setBrushNpcHueState] = useState(editorState.brushNpcHue);
  const [brushNpcScript, setBrushNpcScriptState] = useState(editorState.brushNpcScript);
  ...
  // ── NPC handlers ───────────────────────────────────────────────
  const handleBrushNpcPaletteChange = useCallback((palette: number) => {
    editorState.brushNpcPalette = palette;
    setBrushNpcPaletteState(palette);
  }, [editorState]);

  const handleBrushNpcHueChange = useCallback((hue: number) => {
    editorState.brushNpcHue = hue;
    setBrushNpcHueState(hue);
  }, [editorState]);

  const handleBrushNpcScriptChange = useCallback((script: import('../office/types.js').NpcAction[]) => {
    editorState.brushNpcScript = script;
    setBrushNpcScriptState(script);
  }, [editorState]);

  const handleNpcRemove = useCallback((id: number) => {
    const os = getOfficeState();
    const layout = os.getLayout();
    const currentNpcs = layout.npcs || [];
    const nextNpcs = currentNpcs.filter(n => n.id !== id);
    if (nextNpcs.length !== currentNpcs.length) {
      applyEdit({ ...layout, npcs: nextNpcs });
    }
  }, [getOfficeState, applyEdit]);
  ...
  const handleEditorTileAction = useCallback(
    (col: number, row: number) => {
      ...
      if (editorState.activeTool === EditTool.TILE_PAINT) {
        ...
        editorState.selectedFurnitureUid = hit ? hit.uid : null;
        setEditorTick((n) => n + 1);
      } else if (editorState.activeTool === EditTool.NPCS) {
        if (col < 0 || col >= layout.cols || row < 0 || row >= layout.rows) return;
        
        // Find existing NPC at this tile
        const existingNpcId = os.getCharacters().find(ch => ch.isNpc && ch.tileCol === col && ch.tileRow === row)?.id;
        
        if (existingNpcId !== undefined) {
          // If we clicked on an existing NPC, remove it (like eraser)
          handleNpcRemove(existingNpcId);
        } else {
          // Otherwise place a new one, if walkable
          const isBlocked = os.blockedTiles.has(`${col},${row}`);
          if (!isBlocked) {
            const currentNpcs = layout.npcs || [];
            let minId = -100000;
            for (const n of currentNpcs) {
              if (n.id <= minId) minId = n.id - 1;
            }
            const newNpc = {
              id: minId,
              palette: editorState.brushNpcPalette,
              hueShift: editorState.brushNpcHue,
              script: structuredClone(editorState.brushNpcScript),
            };
            const newLayout = { ...layout, npcs: [...currentNpcs, newNpc] };
            applyEdit(newLayout);
          }
        }
      }
    },
    [getOfficeState, editorState, applyEdit, maybeExpand, saveLayout, handleNpcRemove],
  );
  ...
  return {
    ...
    brushNpcHue,
    brushNpcScript,
    handleBrushNpcPaletteChange,
    handleBrushNpcHueChange,
    handleBrushNpcScriptChange,
    handleNpcRemove,
  };
}
