...
import { API_AGENT_NAMES, DELEGATE_LABEL_MAX_CHARS, SPEECH_BUBBLE_DURATION_SEC } from '../constants.js';
...
/** Short, human display name for an agent id in the activity feed / bubbles. */
function agentDisplayName(os: OfficeState, id: number): string {
  const ch = os.characters.get(id);
  return ch?.agentName || ch?.folderName || `Agent ${id}`;
}

/** Title-case a detected agent name ("son-le" → "Son Le"). */
  ...
  useEffect(() => {
    ...
    const handler = (msg: any) => {
      ...
      if (msg.type === 'layoutLoaded') {
        ...
        os.pushActivity(`${agentDisplayName(os, id)} arrived`);
        saveAgentSeats(os);
      } else if (msg.type === 'agentClosed') {
        const id = msg.id as number;
        os.pushActivity(`${agentDisplayName(os, id)} left`);
        setAgents((prev) => {
          const next = prev.filter((a) => a !== id);
          // Office emptied — reset the summary feed so it doesn't linger after quit.
          if (next.length === 0) os.clearActivity();
          return next;
        });
        ...
        for (const id of incoming) {
          ...
          pendingAgents.push({
            ...
            isExternal: externalAgents[id] || false,
          });
        ...
        // Project rule — chat-driven naming: when the agent writes an
        // `assistant-name-<X>.md` memory file, adopt <X> as its office name and
        // persist it to ~/.pixel-agents/agent-names.json (the name-file convention),
        // so the chosen name survives reloads and future sessions.
        const nameMatch = /assistant-name-([A-Za-z0-9 _-]+)\.md/i.exec(status);
        if (nameMatch) {
          const detected = titleCase(nameMatch[1].replace(/[-_]+/g, ' ').trim());
          const named = os.characters.get(id);
          if (detected && named) {
            named.agentName = detected;
            void fetch(API_AGENT_NAMES, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ id, name: detected }),
            }).catch(() => {
              // Best-effort persistence; the in-memory name still applies.
            });
          }
        }
        ...
        if (isSubagentToolName(toolName)) {
          ...
          const shortLabel = truncate(label, DELEGATE_LABEL_MAX_CHARS);
          // Live delegation: the parent announces the task, the feed narrates it.
          os.showSpeech(id, `Delegating: ${shortLabel || 'a subtask'}`, SPEECH_BUBBLE_DURATION_SEC);
          os.pushActivity(`${agentDisplayName(os, id)} → ${shortLabel || 'subtask'}`);
          if (!runInBackground || !parentHasTeam) {
            const subId = os.addSubagent(id, toolId);
            os.showSpeech(subId, 'On it…', SPEECH_BUBBLE_DURATION_SEC);
          ...
          os.pushActivity(`${agentDisplayName(os, id)}: ${status}`);
        }
        ...
        if (status === 'waiting') {
          ...
          playDoneSound();
          // Turn completed (done chime). Skip the awaiting-input case — that's
          // an idle state and would spam the feed.
          if (msg.awaitingInput !== true) {
            os.pushActivity(`${agentDisplayName(os, id)} finished`);
          }
        }
        ...
        if (subId !== null) {
          ...
          // Let the sub-agent "narrate" the work it was delegated.
          os.showSpeech(subId, truncate(status, DELEGATE_LABEL_MAX_CHARS), SPEECH_BUBBLE_DURATION_SEC);
        }
