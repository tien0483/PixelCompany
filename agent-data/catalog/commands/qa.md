...
If the config overlay date is more than 90 days old, mention: "Your `/qa` config is over 90 days old — consider running `/manager-setup qa` to refresh it."
...
> **Tip:** Run `/manager-setup qa` to generate a repo-specific config that skips browser detection, bakes in your tech stack, and adds framework-specific QA checks.
