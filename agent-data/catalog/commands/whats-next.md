...
If the config overlay date (in the `# Generated by` header) is more than 90 days ago, mention: "Your `/whats-next` config is over 90 days old — consider running `/manager-setup whats-next` to refresh it."
...
> "Run `/manager-setup whats-next` to generate a repo-specific config — future runs will skip discovery and be faster. Or run `/manager-setup all` to configure `/whats-next`, `/qa`, `/ux`, `/dcr`, and `/docs-sync` together."
