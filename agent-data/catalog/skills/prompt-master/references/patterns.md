...
| 27 | **Adding CoT to reasoning models** | "think step by step" sent to a reasoning-native model | Remove it — reasoning models think internally, CoT instructions degrade output |
...
| 36 | **Vague first turn on a frontier model** | "fix the auth bug" with no scope, no files, no criteria | Current models read prompts literally — they no longer fill implicit context. Use Template M. Front-load intent, file scope, constraints, and acceptance criteria. |
