...
| [M — Task Brief](#template-m--task-brief) | Complex, multi-step, or agentic task on a frontier Claude model |
...
**Important:** Only use CoT for standard reasoning models (Claude, GPT-5.x, Gemini). Do NOT add CoT instructions to o3, o4-mini, or reasoning-native modes — they reason internally and CoT instructions degrade their output.
...
## Template M — Task Brief

*Use for any complex, multi-step, or agentic task on a frontier Claude model — claude.ai, API, or Claude Code. Current models read prompts literally and produce narrow output when context is missing. This template front-loads everything so the first turn is the only turn.*
...
**When to use:** any frontier Claude model on any surface — claude.ai, API, Claude Code — when the task is complex, multi-file, ambiguous, or agentic. Not needed for simple one-shot tasks.
